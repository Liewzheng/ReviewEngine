...
### Added
- **Dashboard activity-trend card: 24H/daily granularity toggle with a dark-theme chart restyle (RENG-50)**: `GET /api/v1/dashboard` gains `trendDaily` — 30 points, one per local calendar day for the last 30 days INCLUDING today (today is the final partial day), ordered oldest → newest like `trend`; `time` is the day's local midnight as a unix timestamp (the same `day_start`/`local_midnight` server-local anchor math as the KPI today/yesterday windows, bucketed by calendar day so 23h/25h DST days are still one bucket), `value` counts reviews whose `created_at` falls in `[midnight, next midnight)` regardless of status. The 24h `trend` series is unchanged; both series share ONE bounded window row fetch (newest ≤500 rows from 29 days ago), so the 60s poll pays for a single window query instead of two. The no-store default response serves the same 30 zero points. WebUI: the trend card header gains a capsule `el-segmented` toggle (`24H` / 按天, session-only state — switching never refetches since both series ride the same payload); the daily view renders a histogram series (bright violet, solid fill — lightweight-charts v5 has no native rounded tops) with sparse `M/D` axis labels, the 24H view keeps the line series; both views pin the Y floor to 0 via a series `autoscaleInfoProvider` so all-zero windows hug the floor instead of the ±ε auto-range; grid/axis/series colors brighten to the new `--chart-grid`/`--chart-text`/`--chart-line`/`--chart-bar` theme vars (both themes); a dark floating hover card shows the window label (`2026-09-09` for daily, `14:00 – 15:00` for 24H) with the localized review count; the footer total follows the active view (24h sum vs 30-day sum); the TradingView attribution logo is hidden via the supported `layout.attributionLogo: false` (v5 ≥ 4.1). (`src/server/api/dashboard.rs`, `frontend/src/views/Dashboard.vue`, `frontend/src/types/dashboard.ts`, `frontend/src/services/dashboard.ts`, `frontend/src/style.css`, `frontend/src/i18n/locales/*` ×6, `docs/rest-api.md` §8)
