...
### Fixed
- **Stale cached `index.html` left the frontend on a blank page after an upgrade**: the built SPA ships content-hashed asset filenames, but `serve` returned `index.html` with no `Cache-Control` header at all (tower-http's `ServeDir` only sets `content-type`/`last-modified`), so browsers cached the entry HTML heuristically — after an upgrade, the stale HTML referenced hashed chunks that no longer exist on the server and the app rendered a white screen. A cache-control middleware now stamps the static-file responses: `index.html` (`/` and `/index.html`) returns `Cache-Control: no-cache, must-revalidate`, so every load issues a conditional request that `ServeDir` answers with a cheap 304 when unchanged; content-hashed files under `/assets/**` return `public, max-age=31536000, immutable`; every other path (API, health, non-hashed static files) keeps the previous no-header behavior; and the header is applied only to 2xx responses, so a 404 for a not-yet-deployed asset is never cached long-term. Covered by one hermetic integration test (tempdir fixture tree, no real frontend build required) and three path-decision unit tests. (`src/server/router.rs`, `tests/server.rs`)
