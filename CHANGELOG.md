...
### Fixed
- **Server-side reviews now adjudicate: the false-positive filter fetches the cited file through the provider API instead of requiring a local checkout (RENG-31)**: the final adjudication pass — the layer that re-reads a finding's full cited file with the lead model and drops claims the code disproves — only ever read from a local checkout, while webhook/API reviews fetch their diff through GitLab/GitHub, never clone, and carry the provider slug (`group/project`) in `project_path`. The pass therefore bailed out wholesale: on the deployed instance it ran 17 times in one 4-hour window, 14 times with `Adjudication: no local checkout at …`, and dropped **0 findings in every pass** — which is why High-severity, 10/10-confidence notes landed on claims the author had already disproved (`reports/reng-59-comment-inventory.md` §4.2). Ground truth now comes from a small seam, `src/team/file_source.rs` (`trait FileSource { async fn read_file(path) -> Result<String, FileReadError> }`), with two implementations: `LocalFileSource` over a checkout (unchanged behaviour, still the default whenever `project_path` is a directory) and `ProviderFileSource`, which reuses the existing clients and credential plumbing — GitLab `GET /projects/:id/repository/files/:urlencoded_path/raw?ref=<sha>`, GitHub `GET /repos/:owner/:repo/contents/:path?ref=<sha>` with `application/vnd.github.raw+json` — at the reviewed commit SHA. The token and SHA are the ones the diff fetch already used, resolved at each call site and plumbed through `run_experts` as an `Option<Arc<dyn FileSource>>`: the GitLab and GitHub webhook handlers (`src/server/mod.rs::run_review_common`), the REST review API (`review::resolve::resolve_source`, which hands the adjudicator the provider source for `gitlab_mr` and the checkout itself for `local_repo`), and the CLI/python remote-MR entry point (`src/lib.rs::run_review`); local CLI reviews pass `None` and keep reading their checkout. When a review cannot supply a source (no token, no SHA, unusable URL) the missing input is logged with its consequence instead of degrading silently. (`src/team/file_source.rs`, `src/team/adjudicator.rs`, `src/git_provider/mod.rs`, `src/git_provider/{gitlab,github}/client.rs`, `src/team/orchestrator/{pipeline,mod}.rs`, `src/lib.rs`, `src/server/mod.rs`, `src/server/api/review/resolve.rs`, `src/cli/handlers/review.rs`)
- **Adjudication ground truth is fetched once per (path, revision), bounded, and fails open with a reason (RENG-31)**: cited files are prefetched before any LLM call into a per-pass cache keyed by `(path, revision)`, so a finding-dense review pays one read per file (a repeated failure is not retried either), reads run at most 4 at a time, each under a 30 s ceiling, and the whole prefetch phase under 120 s so a slow provider cannot eat the review's own timeout budget (600 s on the API path) — whatever is still unfetched when the budget is gone counts as an unfetchable file. Every failure keeps the whole affected group of findings unchanged and names the file and reason: HTTP 404 ("not present at the reviewed revision") or any transport/decode failure gets one WARN per file, while 401/403 is recognised as a credential-level failure and gets exactly one WARN for the pass naming the missing read permission (GitLab `read_repository` / GitHub contents read) rather than one per cited file. The diff patch is still never a substitute: when no source exists at all, or a file cannot be fetched, the findings pass through rather than being judged against ±3 lines of context. (`src/team/adjudicator.rs`, `src/team/file_source.rs`)
- **The adjudication log line says which source ran and what it did (RENG-31)**: the pass ends with `Adjudication: source=<local|provider-api> files=N fetches=N dropped=N kept=N` — `files` counts the cited files whose full content was obtained, `fetches` the reads actually performed, `kept` the candidates left untouched — so a production webhook review reads `Adjudication: source=provider-api files=3 fetches=3 dropped=1 kept=2` instead of the old `no local checkout … 0 dropped`. The fail-open line (no local checkout and no provider source) and the per-file/per-credential warnings are unchanged in intent and keep naming the file and reason. (`src/team/adjudicator.rs`)
- **Tests for the remote path, including wiremock**: `test_adjudicate_with_provider_source_drops_false_positive_without_checkout` (a slug for `project_path`, the file read through the provider source: the disproved claim is dropped, the valid one kept, `source=provider-api files=1 dropped=1` logged), `test_adjudicate_over_the_gitlab_api_without_a_checkout` (the real GitLab client against a mock API: one GET at the reviewed SHA, `expect(1)`, and the filter drops the finding), the fail-open trio `test_adjudicate_provider_failure_fails_open_and_warns` (transport failure and 404 → finding kept, WARN naming the file and reason, no LLM call), `test_adjudicate_unauthorized_source_warns_once_and_keeps_everything` (401 → one credential warning for the pass, zero per-file "skipping" lines, everything kept), `test_adjudicate_fetches_each_file_once_per_pass` (7 findings spanning 2 batches on one file + a twice-cited failing file → exactly 3 reads), plus `test_adjudicate_local_source_reads_each_file_once` asserting `source=local files=2 fetches=2`, `test_adjudicate_no_local_checkout_slug_skips_all_candidates` (no source → nothing dropped, no LLM call, one WARN) and the `src/team/file_source.rs` suite (path validation, non-UTF-8, 404/401/403/transport mapping through the real client, factory validation and provider selection). The 14 pre-existing adjudicator tests that drive a local checkout keep passing unchanged. (`src/team/adjudicator.rs`, `src/team/file_source.rs`)

### Changed
- **The provider file read preserves its HTTP status (RENG-31)**: `fetch_file_raw` on both clients is now a thin `anyhow` wrapper over `fetch_file_raw_checked`, which returns `FileFetchError { status, message }` — the same messages as before, plus the status the adjudicator needs to distinguish "absent at this revision" from "this credential may not read the repository". Transport, decode and rejected-path failures carry no status. (`src/git_provider/mod.rs`, `src/git_provider/{gitlab,github}/client.rs`)
- **Docs**: `docs/config-schema.md` now documents the `[report]` adjudication knobs (`adjudicate`, `adjudicate_min_severity`), and `docs/integrations/gitlab.md` / `docs/integrations/github.md` state that server-side reviews adjudicate against the file fetched from the provider at the reviewed SHA, with the fail-open rules (`docs/config-schema.md`, `docs/integrations/gitlab.md`, `docs/integrations/github.md`)
## [0.10.21] - 2026-09-15

### Security
- **Bumped `rustls` 0.23.41 → 0.23.45 (RUSTSEC-2026-0285)**: the advisory published 2026-09-14 (TLS 1.3 handshake messages accepted across encryption-level boundaries; patched in ≥ 0.23.45) failed `cargo audit` on every branch. `rustls-webpki` moved to 0.103.15 with it. No code change.

### Fixed
- **The LLM Status page's per-provider statistics are the recorded usage, not hardcoded zeros (RENG-56)**: `GET /api/v1/llm/providers` filled `errorRate`, `requestCount`, `usagePercent` and `sparkline` with `0.0` / `0` / `0` / `[]` — no code path ever wrote them, so a provider that had served hundreds of reviews was reported as never used, and the "0 % capacity" bar was a limit no configuration expresses (RENG-36 had already replaced the neighbouring lie, `status`/`latencyMs`/`lastChecked`, with a real probe). The card now reports what the reviews themselves recorded: `reviews.llm_summary` (the `[{provider, model}]` snapshot RENG-38 writes on every completed review) aggregated over a rolling 7-day window, at review-level granularity — `requestCount` is the number of reviews that used the provider (one per review whatever the model mix, which is the number a user can reproduce from `GET /api/v1/reviews`), `usageShare` its slice of all usage recorded in the window, `successRate` the completed fraction of the reviews that used it and reached an outcome, and `lastUsedAt` the newest recorded use. The window travels with the numbers (`usageWindowDays`, `usageSince`, `usageAvailable`) so the page labels what it shows. Cost is one aggregate read per request, an index range scan over `reviews.created_at` with `llm_summary IS NULL` and out-of-window rows filtered in the same scan; the JSON is parsed in Rust so SQLite and PostgreSQL need no JSON dialect split. New tests: `aggregate_llm_usage_counts_reviews_per_provider`, `aggregate_llm_usage_skips_rows_without_a_usable_provider`, `aggregate_llm_usage_keeps_counts_with_an_undecodable_timestamp`, `aggregate_llm_usage_empty_window_is_empty`, `llm_usage_since_aggregates_the_window_and_holds_the_boundary` (the bound is inclusive, one microsecond earlier is excluded), `llm_usage_since_is_empty_without_recorded_usage`, `window_start_is_seven_rolling_days`, `usage_share_is_over_the_whole_window`, `empty_window_reports_zero_counts_and_no_derived_metrics`, `success_rate_needs_a_terminal_outcome`, `shares_are_rounded_to_four_decimals`, `provider_items_report_recorded_usage`, `provider_items_empty_window_reports_zero_count_and_null_derivations`, `get_providers_reports_recorded_usage_from_the_store` and `get_providers_reports_null_derivations_on_an_empty_store` (store → handler payload), `get_providers_without_a_store_reports_unknown_usage`. (`src/store/traits.rs`, `src/store/rows.rs`, `src/store/sqlx.rs`, `src/server/api/llm_usage.rs`, `src/server/api/llm.rs`, `src/server/api/mod.rs`, `src/server/task_queue.rs`, `frontend/src/types/llm.ts`, `frontend/src/services/llm.ts`, `frontend/src/composables/useLlmStatus.ts`, `frontend/src/components/Config/ProviderConfigCard.vue`, `frontend/src/views/LlmStatus.vue`, `frontend/src/i18n/locales/*`)

### Added
- **The window total travels with the numbers (RENG-56)**: `usageTotal` is every usage recorded in the window across all provider names — the denominator of each card's `usageShare` — so the page can state what the shares are taken against instead of summing the visible cards, which can only see the providers configured today (a recorded provider may since have been removed, and the two numbers would otherwise contradict each other). The page's "recorded usages" KPI reads it, `null` when the history is unreadable. (`src/server/api/llm.rs`, `frontend/src/services/llm.ts`, `frontend/src/composables/useLlmStatus.ts`, `frontend/src/views/LlmStatus.vue`, `design/llm.md`, `docs/rest-api.md`)

### Removed
- **No fabricated metric is left on the card (RENG-56)**: `usagePercent` (no quota/capacity concept exists anywhere in the configuration) and `sparkline` (no call-level time series is recorded before RENG-57) are gone from the payload, and the hardcoded `errorRate` is replaced by the review-outcome-based `successRate`. A metric with no basis is `null` on the wire and `—` on the page — never a stand-in `0` — which also covers `lastChecked` for a provider that was never probed (it used to be stamped with the current time). The page's "avg latency" KPI shows `—` instead of `0 ms` when no provider was probed, and the request/success metrics are no longer hidden for an `offline` provider: its recorded usage is real even while its key is missing. New tests: `provider_items_omit_the_fabricated_metric_fields`, `provider_items_without_a_store_report_null_usage`, `provider_items_report_probed_status_not_key_presence` (offline / missing report ⇒ `lastChecked: null`). Docs: `docs/rest-api.md` §7 (payload, the `null`-not-`0` rule, the `successRate` caveat), `design/llm.md` §2.2/§3.1/§3.3 (what each number means, why the sparkline is not drawn). i18n: the dead `llm.usage` / `llm.metrics.errors` keys are removed and the new `llm.metrics.successRate`, `llm.usageShare`, `llm.usageWindow`, `llm.lastUsed`, `llm.stats.totalRequestsWindow` keys are translated in all six locales.

## [0.10.20] - 2026-09-14

### Added
- **`serve --data-dir <path>` puts every persisted artifact in one directory (RENG-37)**: running several instances used to mean hacking `HOME`, because the state was scattered over `~/.config/review-engine` — and `REVIEW_DISPATCH_STATE`-style variables could only move one file at a time. The new flag (env form `REVIEW_DATA_DIR`) names a root that is created if missing and made absolute, and every default state path now derives from it through one resolver, `src/paths.rs`: the embedded SQLite database (`review.db`), its at-rest key (`secrets.key`), `ui-state.toml`, `auth.toml`, `dispatcher-state.json`, `feedback.json`, `models-dev-cache.json`, `logs.ndjson`, the `reports/` output directory (`report.output_dir`'s serde default) and the user-level `.code-audit-config.toml`. Without the flag the resolution is unchanged: `--data-dir`/`REVIEW_DATA_DIR` > `REVIEW_ENGINE_CONFIG_DIR` > `~/.config/review-engine`, and a per-artifact full-path variable (`REVIEW_UI_STATE_FILE`, `REVIEW_AUTH_FILE`, `REVIEW_DISPATCH_STATE`, `REVIEW_FEEDBACK_PATH`, `REVIEW_MODELS_DEV_CACHE`, `DATABASE_URL`) still wins for its own artifact — those are operator-written paths, so they outrank the root, and because they are process-wide `serve` now logs one warning per hit at startup (`<VAR> is set — <artifact> stays outside the data dir (<path>)`) instead of silently running a half-isolated instance. The flag is `serve`-only, applied in `main` *before* the log collector opens `logs.ndjson`. Two instances with different `--data-dir` values share no database, key, dedup state or log file: verified end-to-end by `tests/cli/data_dir.rs` (`two_instances_with_different_data_dirs_share_no_state`, which starts two real servers under one `HOME` and asserts each writes its own `review.db`/`secrets.key`/`logs.ndjson` while `~/.config/review-engine` is never created, and `without_the_flag_state_still_lands_in_the_default_config_dir`), by the resolution tables in `src/paths.rs` (`every_default_artifact_lands_under_the_data_dir`, `without_a_data_dir_the_root_is_the_pre_flag_default`, `the_data_dir_outranks_the_config_dir_env`, `an_explicit_artifact_env_var_wins_over_the_root`, `two_data_dirs_resolve_disjoint_state`, `artifact_resolvers_agree_with_the_shared_layout`) and by `serve_data_dir_parses_and_repoints_the_whole_state_layout` in `src/cli/tests.rs`. (`src/paths.rs`, `src/cli/app.rs`, `src/cli/commands.rs`, `src/main.rs`, `src/server/api/config/persist.rs`, `src/server/auth.rs`, `src/server/dispatcher.rs`, `src/server/log_collector.rs`, `src/catalog/mod.rs`, `src/feedback.rs`, `src/models/config.rs`, `src/config/resolver/user_fallback.rs`, `src/cli/handlers/{review,config/provider}.rs`, `tests/cli/data_dir.rs`)

### Changed
- **State-path owners are unified on the state dir (RENG-37)**: `logs.ndjson` and the user-level `.code-audit-config.toml` used to resolve from `$HOME` even when `REVIEW_ENGINE_CONFIG_DIR` was set, so the log file was the one artifact that ignored an operator's config-dir override (inside the shipped images it landed on `/app/.config`, the image layer, and was lost on every recreate — the same class of failure RENG-62 fixed for the dispatch state). Both now resolve through the state dir, so with `REVIEW_ENGINE_CONFIG_DIR=/app/config` the log file lands on the mounted config volume as `/app/config/logs.ndjson`; non-container and non-overridden runs are unaffected. `deploy/gitlab-ee.md`'s log-tailing commands follow the state dir. (`src/server/log_collector.rs`, `src/config/resolver/user_fallback.rs`, `src/cli/handlers/review.rs`, `src/cli/handlers/config/provider.rs`, `src/models/config.rs`, `deploy/gitlab-ee.md`)
- **Docs**: `docs/configuration.md` gains a "Data directory (`serve --data-dir`)" section — the full artifact table, the precedence rules including the per-artifact escape hatch, and a worked example of two isolated instances (ports + `--data-dir`, plus the per-instance volume compose form); the dispatch-state table and `deploy/gitlab-ee.md` now reference the state dir. (`docs/configuration.md`, `docs/config-schema.md`, `docs/migration-0.10.md`, `docs/rest-api.md`, `docs/webhook-dispatch-design.md`, `design/persistence.md`, `deploy/gitlab-ee.md`)
## [0.10.19] - 2026-09-14

### Added
- **Inline-note delivery is now policy-driven and change-type aware (RENG-63)**: RENG-60 made the inline *set* correct (consolidated, deduplicated, adjudicated, inside the diff) but every Critical/High finding still became a note — 321 eligible findings produced 214 inline notes, 87 % of the notes were High-or-above while the board's own severity mix was 45 % medium, 57 % of the corpus notes were noise, 25 were out-of-scope demands on prose or CI glue, and the worst case was **2 500 comments per 1 000 changed lines on a two-line MR** (`reports/reng-59-comment-inventory.md` §3.1/§4.7/§5.3). A round's findings now pass a `PublishPolicy` (`src/publisher/policy.rs`, constructed in code with documented defaults, no new configuration section) before anything is posted: **severity ≥ `high`**, **confidence ≥ 8/10** (the real `u8` field, not a string), **a non-empty recommendation** (the corpus' out-of-scope class was largely asks with no actionable content), and the existing **anchor inside the reviewed diff** requirement. Findings below the policy are never dropped — they stay in the board, whose per-expert sections are rendered from `output.reports` and are untouched — and the pass now reports `policy_excluded` separately from anchor-ineligible and transport-failed. The four knobs are overridable per process with `REVIEW_PUBLISH_MIN_SEVERITY`, `REVIEW_PUBLISH_MIN_CONFIDENCE`, `REVIEW_PUBLISH_MAX_INLINE_NOTES` and `REVIEW_PUBLISH_INLINE_ON_DOCS_ONLY`; a malformed value is ignored with a warning and the default kept, so a typo can never silently disable inline notes, and the effective policy is logged once per publish (`Inline-note delivery policy: severity >= high, confidence >= 8/10, 2 inline note(s) per round, an actionable recommendation, docs/CI-only changes summary-only`). (`src/publisher/policy.rs`, `src/lib.rs`)
- **A per-round inline cap of 2, with the remainder rolled up into the board (RENG-63)**: a round may post at most `REVIEW_PUBLISH_MAX_INLINE_NOTES` (default 2) notes, and the findings that pass the policy but do not make the cut are reported **in the board** instead. The board gains an `## Inline notes — delivery policy` section that names each rolled-up finding (severity, `` `path:line` ``, title, confidence) and states the rule that withheld it; it is rendered only when there is something to report, so an unaffected board is unchanged. Which findings survive the cap is a documented, deterministic total order — **severity descending, then confidence descending, then file path ascending, line ascending, title ascending, expert name ascending** — so the same findings always select the same notes, and the posted notes keep the pre-RENG-63 order (the admitted subset of the consolidated set, in its order). (`src/publisher/policy.rs`, `src/publisher/board.rs`, `src/publisher/mod.rs`, `src/lib.rs`)
- **A documentation/CI-only change is published summary-only — board comment, zero inline notes (RENG-63)**: "code-quality doctrine on prose and CI glue" is where the corpus' `AGENTS.md` / `README.md` / `build.sh` notes came from (25 out-of-scope inline notes), so a change set whose *every* changed file is documentation or CI configuration now gets the board only. The predicate is one readable, unit-testable function of a file list (`src/publisher/policy.rs::is_docs_or_ci_path`) — `.md`/`.mdx`/`.rst`/`.adoc`/`.asciidoc`; anything under `docs/`, `doc/`, `documentation/`, `man/`; license/notice files (`LICENSE`, `LICENCE`, `COPYING`, `NOTICE`, `AUTHORS`, `CONTRIBUTORS`, also `LICENSE-MIT`-style); anything under `.github/`, `.gitlab/`, `.circleci/`, `.buildkite/`, `.woodpecker/`, `.travis/`, `.ci/`, `ci/`; and the CI files `.gitlab-ci.yml`, `.travis.yml`, `azure-pipelines.yml`, `appveyor.yml`, `.drone.yml`, `buildkite.yml`, `.woodpecker.yml`, `codecov.yml`, `Jenkinsfile`. `config.toml` deliberately does **not** qualify — shipped configuration is reviewed as code — the file list comes from the same diff the anchor gate uses (including pure deletions, which have no anchor line), an unknown change set (diff unavailable) is never downgraded, and `REVIEW_PUBLISH_INLINE_ON_DOCS_ONLY=true` restores inline delivery for docs-only rounds. (`src/publisher/policy.rs`, `src/publisher/mod.rs`, `src/lib.rs`)
- **The inline publish pass reports every decision (RENG-63)**: `PublishSummary` keeps its RENG-60 fields and adds `policy_excluded`, `rolled_up` and `summary_only`; the per-pass log line becomes `Inline notes: N posted, N rolled up (board only), N policy-excluded, N anchor-ineligible, N skipped, N failed (N findings considered)`, with a `— documentation/CI-only change: summary only, no inline notes` suffix when that is why nothing was posted. The board assembly moved into the publisher too (`src/publisher/board.rs::render_board`), so the board is built from one place that both the publish path and the tests drive. New tests: `default_policy_documents_its_floors`, `admits_applies_every_floor`, `recommendation_requirement_can_be_switched_off`, `apply_overrides_reads_every_knob`, `apply_overrides_fails_open_on_a_malformed_value`, `docs_or_ci_paths_are_recognised`, `code_and_config_paths_are_not_docs_or_ci`, `docs_or_ci_only_requires_every_path_to_qualify`, `plan_caps_the_round_and_rolls_the_rest_up`, `plan_ranks_by_severity_then_confidence_then_position`, `ranking_is_deterministic_for_equal_keys`, `docs_only_round_selects_no_inline_note_even_for_critical_findings`, `summary_only_board_section_names_the_withheld_findings`, `board_section_is_empty_when_nothing_was_withheld`, `board_section_lists_the_capped_findings`, `test_policy_floors_exclude_low_value_findings_but_keep_them_on_the_board`, `test_per_round_cap_posts_exactly_n_and_reports_the_remainder`, `test_docs_only_round_posts_no_inline_notes_but_a_summary`, `test_mixed_change_set_is_not_docs_only`, `test_cap_selection_is_deterministic`, `test_end_to_end_two_inline_four_rolled_into_the_board` (six admitted findings → 2 inline + 4 on the board), plus three wiremock end-to-end tests driving the real `publish_review_with_diff` path (`publish_review_caps_inline_notes_and_rolls_the_rest_into_the_board`, `publish_review_publishes_a_docs_only_round_summary_only`, `publish_review_posts_inline_notes_for_a_mixed_change_set`). Docs: `docs/integrations/gitlab.md`, `docs/integrations/github.md` (delivery policy sections) and `docs/configuration.md` (env overrides and the docs/CI predicate). (`src/publisher/policy.rs`, `src/publisher/board.rs`, `src/publisher/mod.rs`, `src/lib.rs`, `docs/integrations/gitlab.md`, `docs/integrations/github.md`, `docs/configuration.md`)

## [0.10.18] - 2026-09-14

### Fixed
- **A provider's health now follows its credentials: breaking a key can no longer leave the LLM page showing `healthy` (RENG-36)**: `GET /api/v1/llm/providers`, the dashboard's `health.llmProviders` and `GET /api/v1/system/health`'s `llmProviders` all derived their verdict from "the `api_key` field is non-empty" — no probe was ever made, so editing a key in the Web UI while the service kept running left the LLM Status page green while every review failed with 401 (the RENG-34 E2E finding). Provider health now comes from one shared probe cache, `src/server/api/llm_health.rs` (`AppState::llm_health`): the same `GET {api_base}/models` check the Test Connection button runs, cached per provider for 60 s. `healthy` is only ever produced by a successful probe, a failed probe reports `error` with the transport/HTTP reason (e.g. `HTTP 401 Unauthorized`), a provider without a key is `offline` and is never probed, and there is no "assume healthy" fallback. The card payload's `status` / `latencyMs` / `lastChecked` now carry the probe's real values instead of `latencyMs: 0`, and `overall` (dashboard and `/system/health`) follows the verdicts — all healthy → `success`, some → `warning`, none → `error` — instead of key presence. (`src/server/api/llm_health.rs`, `src/server/api/llm.rs`, `src/server/api/dashboard.rs`, `src/server/api/system.rs`, `src/server/state.rs`)

### Added
- **Credential/config changes invalidate the cached health, per provider (RENG-36)**: the cache key is a SHA-256 fingerprint over `provider + model + api_base + api_key`, and every change path (`PUT /api/v1/config`'s `llm` section, which is how providers are edited in the UI today, plus the `POST` / `PUT` / `DELETE /api/v1/llm/providers` endpoints) calls `LlmHealthStore::retain_live`, dropping every entry whose exact config is no longer configured. The next read of any of the three health surfaces then probes the changed provider — never a pre-change leftover — while untouched providers keep their status and are not re-probed: an edit to one provider invalidates one entry, not the whole LLM cache. The fingerprint makes the same guarantee structurally (a changed key cannot match the old entry even if an invalidation call were missed). Reads are cheap: a probe that is missing is awaited (bounded by the probe's own 10 s timeout, i.e. only at boot or right after a change), an entry past its 60 s TTL is served at once and refreshed in the background, and concurrent readers of the same provider collapse into one request through a per-fingerprint flight gate. A manual Test Connection now also records its result, so the page agrees with what the user just saw instead of re-probing. New tests: `credential_change_invalidates_cached_health` (wiremock: healthy key → key broken through `PUT /api/v1/config` → `error` → key restored → `healthy`, asserting one probe per read), `provider_crud_invalidates_the_changed_entry`, `provider_items_report_probed_status_not_key_presence`, `dashboard_health_follows_probe_verdicts`, `system_health_llm_rows_follow_probe_verdicts`, plus the store's own `a_changed_key_is_reported_from_its_own_probe`, `retain_live_drops_only_the_changed_provider`, `fingerprint_tracks_every_probe_relevant_field`, `fresh_entries_are_served_and_stale_ones_refresh_in_background`, `concurrent_readers_share_one_probe_per_provider`, `keyless_provider_is_offline_and_not_probed`, `recorded_probe_is_served_to_the_next_read`, `status_vocabularies_are_stable`. Against the pre-fix code the reproduction test fails exactly as reported (`a broken key must not keep its healthy badge: …"status":"healthy"`), and with the invalidation calls removed the CRUD test fails at the re-add assertion — both verified locally. Docs: `docs/rest-api.md` §7 (`GET /llm/providers`, `POST …/{id}/test`), §8 (`health.llmProviders`) and `GET /api/v1/system/health`, `docs/configuration.md` (LLM page), `design/llm.md` §3. (`src/server/api/llm_health.rs`, `src/server/api/llm.rs`, `src/server/api/dashboard.rs`, `src/server/api/system.rs`, `src/server/api/config/put.rs`, `src/server/state.rs`, `docs/rest-api.md`, `docs/configuration.md`, `design/llm.md`)
## [0.10.17] - 2026-09-14

### Fixed
- **A rejected credential fails fast instead of burning the retry budget (RENG-35)**: the retry decision in `complete_with_fallback` searched the error *text* for `"429"`/`"500"`/`"timeout"`/`"connection"`, so a request the provider had already refused was retried up to 3 times with 1s + 2s of backoff whenever its message happened to spell one of those words — exactly the RENG-34 E2E symptom, where a wrong `api_key` answered 401 and the call still waited out every attempt, and the mirror case of a 4xx whose *body* mentions "connection closed before message completed" or a "500"-looking number. The verdict is now taken from the HTTP status (`LLMClient::retry_verdict`): **408 / 429 / 5xx retry**, **every other 4xx is permanent** (one attempt, no sleep, provider status and reason preserved for the caller), and **an error carrying no status** (transport/DNS/TLS, timeout, unparsable response) keeps the historical retry. The status is read from the first `" returned "` marker of the first message in `err.chain()` that has one — the shape every HTTP failure in this stack is written in (`"<Provider> API returned <status> <reason>: <body>"`, `reqwest::StatusCode` rendered as `401 Unauthorized`) — so body text can no longer promote a credential error into a retry, and an unparsable error is retried rather than silently given up on. A permanent failure is now logged with its status and attempt count (`LLM request failed permanently (401), not retrying … status=401 attempt=1 max_retries=3`) and advances to the next chain config as before, since that entry is a different provider with its own credentials. New tests: `test_retry_verdict_by_http_status`, `test_retry_verdict_reads_the_status_through_context_wrappers`, `test_retry_verdict_is_not_fooled_by_status_like_text`, `test_real_http_401_fails_fast_with_one_attempt`, `test_real_http_403_fails_fast_with_one_attempt`, `test_real_http_401_with_connection_and_500_text_is_not_retried` (the false-positive regression), `test_real_http_429_retries_up_to_the_attempt_budget`, `test_real_http_503_retries_up_to_the_attempt_budget`, `test_500_retries_up_to_the_attempt_budget`, `test_real_transport_failure_without_status_still_retries`, `test_permanent_401_advances_the_chain_without_retrying`, plus the integration test `tests/llm/main.rs::test_permanent_401_fails_fast_then_falls_back_to_the_next_config`, which drives two real HTTP configs and asserts the give-up log, the fallback flag and the preserved RENG-55 chain-advance line. (`src/llm/client/mod.rs`, `src/llm/client/tests.rs`, `tests/llm/main.rs`, `docs/configuration.md`)

## [0.10.16] - 2026-09-14

### Fixed
- **Dispatch state now lives on a mounted volume, so container recreation no longer disarms webhook dedup (RENG-62)**: `MrDispatcher::persistent()` defaults to `~/.config/review-engine/dispatcher-state.json`, which inside the shipped image resolves to `/app/.config/...` — the image layer, not a mount — while neither shipped compose file set `REVIEW_DISPATCH_STATE`. A recreate therefore wiped the file: `docker inspect` shows the container recreated at `00:56:59Z` with `/app/.config` absent from its mounts, and the `02:16:57` webhook burst re-reviewed **seven** MRs whose SHAs had not changed since 09-07…09-11 within six seconds — seven redundant rounds and re-posted comments for unchanged code — while the same window also carries 28 legitimate `already reviewed` skips, i.e. the guard worked and only its persistence failed. Both compose files now set `REVIEW_DISPATCH_STATE: /app/config/dispatcher-state.json` on the `./config` volume they already mount for `REVIEW_ENGINE_CONFIG_DIR` (non-container runs keep the `~/.config` default; an empty value falls back to it, and the resolved path is logged at startup as `Dispatcher: persisting dispatch state to <path>`). The parent directory is created on first write, so a path on a freshly mounted volume works. (`docker-compose.yml`, `deploy/standalone-compose.yml`, `src/server/dispatcher.rs`, `docs/configuration.md`, `docs/integrations/gitlab.md`, `docs/webhook-dispatch-design.md`)

### Added
- **Content-based dispatch gating: an amend that changes nothing is no longer re-reviewed (RENG-62)**: dedup keyed on the commit SHA alone, so a force-push or amend — a new SHA over a near-identical diff (the inventory's worked example, `radxa/…5.15.147!26`, ran three rounds and posted five bot comments over one small change) — still bought a full round and re-posted the same notes. The dispatcher now also records `content_fingerprint` (hex SHA-256 of the diff text the review ran on) and, after the diff is fetched, `claim_content` gates the dispatch on it: identical content is skipped (`skipping: content unchanged (sha A → B)`), changed content is reviewed (`re-reviewing: content changed (sha A → B)`), and the SHA guard remains a strict subset (same SHA → `AlreadyReviewed` before any fetch). The fingerprint is taken from the diff the review already holds — no second `fetch_diff()`. A skipped dispatch now records the new SHA and clears the running marker, so neither the same event nor a delayed webhook can re-enter the gate or wedge the MR for the 15-minute timeout. Policy: webhook push/update events (`merge_request` / PR `opened|reopened|synchronize`) are gated; an explicit user-triggered run (`/review` / `/describe` comment) is `ContentGate::Bypassed` and always reviews, and the REST submit/rerun path never consults the dispatcher at all. Fail-open by design: a first-sight MR, or a state file written before this change (no fingerprint recorded), reviews once and records one from then on. New tests: `state_path_from_prefers_the_env_override`, `persistent_dispatcher_creates_the_configured_state_file`, `content_fingerprint_is_stable_and_content_sensitive`, `decide_content_covers_first_sight_unchanged_and_changed`, `claim_content_skips_unchanged_content_and_finalizes_dispatch`, `claim_content_reviews_when_content_changed`, `claim_content_never_skips_a_first_sight`, `fingerprint_survives_reload_and_gates_amends`, `legacy_state_without_fingerprint_reviews_once_then_records_one`, `complete_without_fingerprint_keeps_the_recorded_one`, `bypassed_gate_never_skips`, `enabled_gate_skips_identical_content`, plus two wiremock-backed tests driving the real provider path (`unchanged_content_update_enqueues_no_review`, `explicit_rerun_bypasses_the_content_gate`). (`src/server/dispatcher.rs`, `src/server/mod.rs`, `src/server/gitlab/hooks.rs`, `src/server/github.rs`)
- **The webhook trigger semantics are documented (RENG-62)**: `docs/integrations/gitlab.md` gains a "When a review is triggered" table covering merge-request `open`/`reopen`/`update`, `/review`-style notes, other notes, push hooks, unknown events and the REST submit/rerun path, each with what happens when the content is unchanged — including the two facts that were previously only in the code: `handle_push_hook` is a logging stub that starts no review (29 `Push hook received` lines, zero reviews in a four-hour production window), and the REST path never consults the dispatcher, so an explicit submit/rerun is never deduplicated away.

## [0.10.15] - 2026-09-14

### Fixed
- **Manual `gitlab_mr` submissions route their MR URL the way webhooks already did (RENG-33)**: `POST /api/v1/reviews` with an MR URL on a host the server cannot reach — the reported case, a URL on the pasted `http://localhost:8929` while the review server runs in a container — was accepted with `202` and only failed inside the async task with an opaque `Failed to send GET`, leaving an `Untitled Review` / `unknown` / `failed` row in the history that the user had to work around by hand-writing `host.docker.internal`. The webhook path never had the problem: it re-hosts the payload's `external_url` onto the matched platform's reachable base before dispatch. The manual path now applies the same rule with the same pieces — `GitPlatformConfig::matches_review_url` (new: the scheme-less `host[:port]` identity of `base_url` OR the entry's configured `internal_base_url`, host case-insensitive, an explicitly written default port folded to absent, otherwise the port compared strictly) feeding the existing `rewrite_url_to_platform` + `review_base_url` helpers the webhook path uses. The rewritten URL is what the review fetches AND what the task records as its MR URL (the submitted URL is not stored; it is logged once with the matched platform), and the whole decision runs before `enqueue_review`, so a rejected submission leaves no history row at all. `rerun` re-applies the same gate, and the credential lookup now resolves the token of a platform that owns either configured address. (`src/models/git_platform.rs`, `src/server/api/review/mr_url.rs`, `src/server/api/review/{handlers,resolve,task,mod}.rs`, `src/server/gitlab/mod.rs`)
- **A local MR host that no platform claims now fails fast with an actionable `400` instead of an opaque late failure (RENG-33)**: when no configured platform matches and the submitted URL's host is a well-known local address (`localhost`, `*.localhost`, any `127.0.0.0/8`, `0.0.0.0`, `::1`, `::`) — inside a container those name the container itself — submission is rejected with `400` and a message naming the host and the two ways out: submit the address the server reaches (in Docker, `host.docker.internal`) or configure the platform's `baseUrl` / `internalBaseUrl` (`internal_base_url`) so the rewrite applies. Malformed URLs keep their existing `422` (the parse gate runs first); a matched platform is always trusted, so a GitLab reachable only on a local address still works. New tests: `submit_gitlab_mr_rewrites_host_onto_matched_platform_internal_base_url` (wiremock: the fetch lands on the rewritten host and the record stores it), `submit_gitlab_mr_with_local_host_and_no_matching_platform_returns_actionable_400`, `unreachable_host_error_names_the_host`, `malformed_gitlab_mr_url_still_returns_422`, `unmatched_reachable_host_passes_through_unchanged`, `submit_gitlab_mr_on_internal_base_url_is_kept_and_routed_to_its_platform`, `rerun_reapplies_the_mr_url_routing_gate`, plus `matches_review_url_includes_configured_internal_base_url`, `find_platform_for_review_url_matches_base_or_internal_regardless_of_verification` and the `mr_url` routing unit tests. Docs: `docs/rest-api.md` §1 (`gitlab_mr` URL 主机改写), `docs/integrations/gitlab.md` (manual submissions). (`src/server/api/review/mr_url.rs`, `docs/rest-api.md`, `docs/integrations/gitlab.md`)
## [0.10.14] - 2026-09-14

### Fixed
- **The Dashboard trend chart follows the light/dark toggle (RENG-51)**: the RENG-50 chart resolves its palette to *concrete* canvas colors with `getComputedStyle` at init (canvas cannot parse CSS `var()`), so flipping the app theme left the chart painted with the previous palette until the 24H/按天 toggle happened to rebuild it — the tooltip, grid, axes and series kept the old theme's colors indefinitely. The theme now lives in one shared module-scope composable, `frontend/src/composables/useTheme.ts` (`isDark` + `setTheme`/`toggleTheme`/`initTheme`, same `<html data-theme>`/`.dark` application as before), which `App.vue`'s sidebar toggle consumes and the Dashboard `watch`es: on a switch the palette is re-resolved and **re-applied** (`chart.applyOptions` for layout/grid/crosshair/price-scale/time-scale borders, `activeSeries.applyOptions` for the series and its crosshair-marker colors) rather than rebuilt, so the series, its data, the crosshair subscription, the integer Y ticks (`priceFormat` precision 0 / minMove 1), the Y-floor-0 autoscale provider, the hidden last-value badge and price line, and `fitContent` are all untouched. Theme switches are rare, so the watcher is event-driven — nothing per-frame. (`frontend/src/composables/useTheme.ts`, `frontend/src/App.vue`, `frontend/src/views/Dashboard.vue`)
- **Stale "30-day" trend-window references left behind by RENG-50**: RENG-50 shrank the daily series to `TREND_DAILY_DAYS = 14` but three descriptions still claimed 30 — the module doc and the `default_trend_daily` doc in `src/server/api/dashboard.rs` ("30-day daily trend", "30 zero points") and the `trendDaily` JSDoc in `frontend/src/types/dashboard.ts` ("30 calendar-day buckets"). All now say 14 / reference `TREND_DAILY_DAYS`; comments and docs only, no behaviour change (`docs/rest-api.md` already carried the 14-day text). Unrelated 30s were left alone (the reaper's 30-minute retention, the scoring system's 30-point critical penalty).
- **Two documents stale since earlier releases (RENG-51)**: `docs/configuration.md` still opened the Web UI section with "The page is read-only until you click **Edit**" and described an edit-mode/save/cancel flow — RENG-41 removed all edit modes, so every field is now always editable and auto-saved (500 ms debounce, header save indicator, poll ticks skipped while dirty or saving); the secret-field sentence was corrected the same way (a secret field only ever shows the `***` mask the API returns, never a stored value). `design/llm.md` §3.2 described a test flow that never shipped (`ElAlert` result banners, a "Refresh All" bulk `POST /llm/test-all` on a wrong endpoint path) — it now documents the shipped per-card probe (`POST /api/v1/llm/providers/{id}/test`, shared `TestResultLine` with dismiss control, page-session state outside the polled list) and states that no bulk test exists; §1/§2/§4.1/§6/§7 were corrected for the same features (no `llm.status` SSE feed — the page polls every 30 s; no card flash/animation; the endpoint list now names the five routes that exist). (`docs/configuration.md`, `design/llm.md`)

## [0.10.13] - 2026-09-14

### Fixed
- **Inline MR/PR notes are published from the consolidated finding set (RENG-60)**: `publish_review` walked `output.reports[].findings` — the raw per-expert lists — while deduplication and adjudication mutate `consolidated.findings` only, so both anti-noise layers were inert on the posting path. The production corpus shows the cost: `duplicates_merged` summed to **1** across 140 reviews while **32 duplicate inline notes** were posted, 23 of them same-round multi-expert restatements of a single issue (`sirena!22` posted four notes — ux, security, database, devops — for "audio write errors are silently ignored"; `sirena!7` three for one `0700` permissions request). Inline publishing now draws from the consolidated set, in its order, so what dedup merged and what the adjudicator dropped is what reaches the MR. (`src/publisher/mod.rs`, `src/lib.rs`)
- **Inline note bodies carry their file anchor (RENG-60)**: the body interpolated expert, title, confidence and recommendation only, so two findings that differed just in `file` rendered byte-identical notes — the `10737`/`10738` pair on `radxa/…5.15.147!26`, posted 2 s apart, were unreadable as two distinct anchors. Bodies now render `` `path:line` `` (or `` `path:start-end` `` for multi-line findings), and a unit test pins that two findings differing only by file can no longer produce the same body.
- **A rejected inline anchor no longer aborts the rest of the notes (RENG-60)**: the posting loop used `?` per finding, so the first `400 line_code can't be blank` (5 occurrences in the 4-hour corpus window; 25 attempts produced 20 notes) silently discarded every remaining note of that expert report. Posting is now per-finding isolated — permanent errors (4xx) are logged and skipped, transient ones (transport failure, 408/429/5xx) are retried up to 3 attempts with a bounded exponential backoff — and the pass ends with a summary line (`posted / skipped / failed / ineligible`) plus a partial-failure error so a lost note is never silent.
- **Findings whose anchor is outside the reviewed diff are no longer sent (RENG-60)**: the review pipeline keeps findings whose line falls outside every changed hunk (annotated `⚠️ line outside diff hunk`), and the publisher posted them anyway, earning the provider's `line_code can't be blank` rejection. `DiffIndex` indexes the changed new-side line ranges of the diff the review actually ran on and gates every candidate note on it; the server path passes that diff directly (no extra API call), and only an unparsable/empty diff disables the gate (fail-open, with a warning) rather than skipping everything. New tests: `test_publish_set_equals_consolidated_set`, `test_publish_inline_notes_isolates_failures`, `test_publish_inline_notes_retries_transient_failures`, `test_publish_inline_notes_bounds_transient_retries`, `test_publish_inline_notes_skips_findings_outside_diff`, `test_inline_body_includes_file_anchor`, `test_diff_index_has_no_anchor_for_pure_deletions`, `test_transient_error_classification`, plus two wiremock-backed tests that drive the real GitLab client (`test_inline_publish_isolates_blank_line_code_rejection`, `test_inline_publish_retries_transient_verdict_from_real_client`) so the retry classification is pinned to the errors the client actually emits. Not addressed here: cross-round re-posts of a finding a later round no longer reproduces (the corpus' 9 cross-round duplicates, 56 stale notes) — that needs a posted-findings ledger and supersede/retract machinery, tracked as RENG-61. (`src/publisher/mod.rs`, `src/lib.rs`, `src/server/mod.rs`)
## [0.10.12] - 2026-09-14

### Fixed
- **One-off operation results survive the auto-refreshing pages' poll ticks (RENG-54)**: clicking a provider card's Test Connection showed the latency briefly and then the LLM Status page's 30 s poll wiped it (`0 ms` → test → `283 ms` → poll → `0 ms`). The result was merged into the provider list, which every tick rebases on `GET /llm/providers` — and that endpoint reports `latencyMs: 0` (real per-provider stats are RENG-56/57's, not shipped yet), so the poll always overwrote the value the user had just asked for. Test results are now page-session state held **outside** the polled model: `frontend/src/composables/useTransientResult.ts` keeps a keyed map that only the test path writes, and `useLlmStatus` records each outcome — a failed probe included, which previously surfaced only as a toast — under the provider's **stable identity** (the provider name the config echo carries as `providers[].provider`, and the key the page's cards use), not under the runtime id from `GET /llm/providers`: that id is composed server-side as `<provider>-<position>` (`src/server/api/llm.rs`), so deleting or reordering any provider shifts the positions behind it and a result keyed by the old id would silently become unreachable — the line would vanish from a card whose provider the user had just tested. The card renders the result below its metrics as an outcome tag (`Connected — 283ms` / `Failed — …`) with the time the probe ran and a ✕ that dismisses it, while the metrics row keeps the server's own numbers, so the live value and the tested value no longer overwrite each other. The old in-place `status`/`latencyMs` mutation (exactly what the next tick undid) is gone, and editing the tested provider clears its result.
- **The Configuration page's Git-platform probe uses the same pattern (RENG-54)**: the row's Test button only raised a toast, and the page's 10 s poll replaces `gitPlatforms` wholesale — a result kept on the row would have gone with the array. Each row now records the probe outcome (`ok` + version, or the error) in the same session store keyed by the platform's name — the row's stable identity in the config (the backend matches secrets on it) rather than an array index — shows it under the row with the same tag / timestamp / dismiss affordance, and clears it when the user dismisses it, re-runs the test, edits or removes the platform. An edit can rename a row, which changes that identity, so an edit clears the result under both the old and the new name: a renamed platform starts with no probe rather than inheriting the previous one. Both pages render it through one shared `TestResultLine` component, so the affordance cannot drift apart. (`frontend/src/composables/useTransientResult.ts`, `frontend/src/components/common/TestResultLine.vue`, `frontend/src/composables/useLlmStatus.ts`, `frontend/src/components/Config/ProviderConfigCard.vue`, `frontend/src/components/Config/GitPlatformsSection.vue`, `frontend/src/views/LlmStatus.vue`, `frontend/src/i18n/locales/*` ×6)
- **Two more places where a tick could overwrite user-initiated state, from the same audit (RENG-54)**: the Experts page's enable switch is optimistic (the PUT is in flight for a moment), but a 10 s tick landing inside that window reconciled the pre-PUT server value back onto the expert, so the switch visibly snapped back and only caught up on a later tick; ticks are now also skipped while an expert PUT is in flight (`savesInFlight`, which replaces the write-only `weightSavePending` and joins the weight-slider debounce in the `isPaused` gate). The Queue Monitor's max-concurrency field resynced from `stats.maxConcurrent` during the 400 ms debounce, before the commit reads the input, so a 3 s tick inside that window silently replaced the value the user had typed; the resync now stands aside while a commit is pending. (`frontend/src/views/ExpertsManagement.vue`, `frontend/src/views/QueueMonitor.vue`)
  - Audited and deliberately left alone: the Queue page's pause/resume/cancel/retry/cancel-all outcomes, the History page's export and copy-id results, and the Config/Experts/LLM save markers are toasts or independent refs that no poll writes; the Dashboard's trend toggle and chart tooltip are local UI state; the Upgrade dialog's progress is server job state the poll is meant to drive; the in-dialog provider test (`ProviderEditDialog`) lives in a dialog the silent poll never unmounts. RENG-41's guarantees are intact — polls stay silent, open dialogs and in-progress edits survive, and the edit guard now covers in-flight saves too.
- **New i18n strings in all six locales (RENG-54)**: `common.lastTest` (`Last test: {date}`) and `common.dismissTestResult` for the shared test-result line; the outcome text reuses the existing `config.llm.connected` / `config.llm.testFailed` and `config.gitPlatforms.testOk` / `config.gitPlatforms.testFailed` keys, which the add/edit dialog and the toasts already use.

## [0.10.11] - 2026-09-14

### Fixed
- **The primary LLM provider selection and the chain order are now authoritative at runtime (RENG-55)**: the Web UI's primary provider was ignored when a review actually ran. `select_llm_config` cloned `configs.first()` as the base for an expert with a custom model, and the review pipeline passed the raw stored provider list — so a deployment with the stored list `[xiaomi, deepseek]` and `deepseek` selected as primary always reviewed on xiaomi (24/24 persisted reviews carried `xiaomi-token-plan-cn/mimo-v2.5` in `reviews.llm_summary`). The runtime chain is now produced in one place — `ordered_llm_configs(primary, configs)` (`src/llm/mod.rs`): the persisted `llm.primaryProvider` first, then every remaining provider in its stored order — and every review-executing entry point takes its providers from `AppState::ordered_llm_configs()` (REST review submit + rerun fallback, repo review, GitLab MR/Note/System hooks, GitHub webhook) instead of reading `state.llm_configs` directly. An empty or stale primary (naming a provider that no longer exists) degrades to the stored order, whose head is the effective primary; an expert with a custom `model` now runs **on the primary** with the model substituted (previously the first stored provider). The two persistence paths were reconciled on the same rule: `load_llm_providers` orders by the stored list index (`raw.position`, written by the save path as the array index — verified end-to-end) instead of `ORDER BY provider`, with rows lacking a position sorting last in `updated_at` order, so the DB order and the UI array order can no longer disagree. (`src/llm/mod.rs`, `src/server/state.rs`, `src/server/api/review/task.rs`, `src/server/api/repo.rs`, `src/server/gitlab/handler.rs`, `src/server/github.rs`, `src/store/sqlx.rs`, `src/store/rows.rs`, `docs/configuration.md`)

### Added
- **Effective-provider visibility on the LLM Status page (RENG-55)**: `GET /api/v1/llm/providers` gains `isPrimary` (the chain head), `chainPosition` (1-based rank in the runtime chain) and `position` (0-based index in the stored list, unchanged by the primary selection) per provider, so the page shows the order reviews actually walk instead of only listing the configuration. Each provider card carries the primary badge plus a quiet chain marker (链序 #1 / Chain #1), and a new 最近使用 / Recent Usage strip below the grid lists the newest reviews' `provider/model` from `reviews.llm_summary` (existing `GET /api/v1/reviews` list endpoint, 8 rows, refreshed on the page's 30s tick) — a review that used none of the chain head's provider is tagged 未使用首选 / Primary not used, which is exactly the production symptom (primary deepseek, reviews on xiaomi) made visible. Six locales for the new strings. (`src/server/api/llm.rs`, `frontend/src/types/llm.ts`, `frontend/src/composables/useRecentLlmUsage.ts`, `frontend/src/views/LlmStatus.vue`, `frontend/src/components/Config/ProviderConfigCard.vue`, `frontend/src/i18n/locales/*` ×6, `design/llm.md`)
- **A fallback that skips the primary is observable (RENG-55)**: `complete_with_fallback` now logs the chain advance at INFO with structured fields (`primary_provider`, `chain_position`, `provider`/`model`, `next_provider`/`next_model`, `reason`) and logs `LLM fallback engaged: the primary provider did not answer…` naming both the skipped primary and the provider that answered; only a failure on the last chain entry stays a WARN. `CompletionResult` gains `fallback: bool`, true when the successful config was a non-head chain entry, so callers can tell a fallback hit from a normal run (the used provider/model continues to be snapshotted into `reviews.llm_summary` by RENG-38). (`src/llm/client/mod.rs`, `src/llm/provider.rs`)
## [0.10.10] - 2026-09-14

### Changed
- **Auto-refreshing pages drop their manual Refresh buttons for a quiet "last updated" marker (RENG-52)**: the Dashboard, Configuration, and Queue Monitor pages poll themselves (60 s / 10 s / 3 s, silently — see 0.10.7), so their Refresh buttons were a second, non-silent path to a fetch that now happens anyway; the buttons are gone and their place in the page header is taken by a small "更新于 12:03" / "Updated 12:03" line.
  - **Removed controls**: Dashboard page-header Refresh button and the System Health card's `RefreshRight` icon button (both called the same full-page refresh), Configuration's Refresh button plus the `useConfigForm.refreshConfig` handler it was the only caller of, and Queue Monitor's Refresh button. Nothing else consumed them: the `kpis.activeQueue` KPI card's `Refresh` glyph and the LLM Status card's `RefreshRight` glyph are decorative icons, and the History/Experts pages never had refresh buttons — the `Refresh` icon left on those pages belongs to the rerun action.
  - **New shared marker** (`frontend/src/components/common/LastUpdated.vue`): renders `{time}` as a short locale-aware wall clock (`12:03`) through the app locale, with deliberately low emphasis — 12 px, `--el-text-color-placeholder` at 0.6 opacity, monospace, no icon, no border — so it reads as metadata next to the 24 px page title in both themes.
  - **Failure visibility**: with the buttons gone, silent polls have no other way to surface a failure, so the marker carries it — a failed tick keeps the last successful time, switches the text to a warning-colored "更新失败 · 12:03" (`common.updateFailedAt`, or `common.updateFailed` when nothing has ever loaded). One state change, no toast, no repeated notification; the Dashboard's existing notification for a failed *initial* load is unchanged.
  - **Timestamp plumbing**: `useDashboard` now exposes `lastUpdated` / `pollFailed` (stamped by both the non-silent initial load and the silent 60 s poll); `useConfig.fetch` and `useQueue.fetchStats` / `fetchTasks` now return a success boolean — the silent path swallows its error by design (it writes neither `error` nor `loading`), so the return value is what lets a view report a failed poll without resurrecting the skeleton or the error banner; `useConfigForm.loadConfig` passes that boolean through. Configuration and Queue Monitor wrap their `useAutoRefresh` closures in tracked loaders that stamp the time on success and flag the failure otherwise. `useAutoRefresh`'s own API is unchanged.
  - **i18n**: added `common.lastUpdated` / `common.updateFailed` / `common.updateFailedAt` to all six locales; removed the now-orphaned `common.refresh`, `dashboard.updatedAt`, `dashboard.refreshAria`, `dashboard.health.refreshAria`, `dashboard.refreshed`, `dashboard.refreshFailed`, `config.refreshedTitle`, `config.refreshed`.
  - Also: the Configuration header no longer disappears wholesale below 768 px — only the transient save-status indicator is dropped there, so the liveness marker stays visible on narrow screens. (`frontend/src/components/common/LastUpdated.vue`, `frontend/src/views/Dashboard.vue`, `frontend/src/views/Configuration.vue`, `frontend/src/views/QueueMonitor.vue`, `frontend/src/composables/useDashboard.ts`, `frontend/src/composables/useConfig.ts`, `frontend/src/composables/useConfigForm.ts`, `frontend/src/composables/useQueue.ts`, `frontend/src/i18n/locales/*` ×6)

## [0.10.9] - 2026-09-13

### Added
- **Dashboard activity-trend card: 24H/daily granularity toggle with a dark-theme chart restyle (RENG-50)**: `GET /api/v1/dashboard` gains `trendDaily` — 14 points, one per local calendar day for the last 14 days INCLUDING today (today is the final partial day), ordered oldest → newest like `trend`; `time` is the day's local midnight as a unix timestamp (the same `day_start`/`local_midnight` server-local anchor math as the KPI today/yesterday windows, bucketed by calendar day so 23h/25h DST days are still one bucket), `value` counts reviews whose `created_at` falls in `[midnight, next midnight)` regardless of status. The 24h `trend` series is unchanged; both series share ONE bounded window row fetch (newest ≤500 rows from 13 days ago), so the 60s poll pays for a single window query instead of two. The no-store default response serves the same 14 zero points. WebUI: the trend card header gains a capsule `el-segmented` toggle (`24H` / 按天, session-only state — switching never refetches since both series ride the same payload) and the granularity-neutral title 活动趋势 / Activity Trend; the daily view renders a histogram series (bright violet, solid fill — lightweight-charts v5 has no native rounded tops) with sparse `M/D` axis labels, the 24H view keeps the line series; both views pin the Y floor to 0 via a series `autoscaleInfoProvider` so all-zero windows hug the floor instead of the ±ε auto-range, format Y ticks as integers (`priceFormat` precision 0 / minMove 1), and drop the series last-value badge (`lastValueVisible: false` — the hover tooltip already shows values); grid/axis/series colors brighten to the new `--chart-grid`/`--chart-text`/`--chart-line`/`--chart-bar` theme vars (both themes), resolved to concrete values via getComputedStyle because canvas cannot parse CSS `var()`; a dark floating hover card shows the window label (`2026-09-09` for daily, `14:00 – 15:00` for 24H) with the localized review count; the footer total follows the active view (24h sum vs 14-day sum); the TradingView attribution logo is hidden via the supported `layout.attributionLogo: false` (v5 ≥ 4.1). (`src/server/api/dashboard.rs`, `frontend/src/views/Dashboard.vue`, `frontend/src/types/dashboard.ts`, `frontend/src/services/dashboard.ts`, `frontend/src/style.css`, `frontend/src/i18n/locales/*` ×6, `docs/rest-api.md` §8)

## [0.10.8] - 2026-09-11

### Fixed
- **Dashboard aggregates from the persistent review store instead of the reaped in-memory queue (RENG-32)**: the Dashboard's KPIs, 24h trend, and recentReviews were aggregated from the in-memory task store, whose reaper deletes completed entries 30 minutes after completion — on the production NAS (114 rows persisted in `reviews`) every figure sat at zero once entries aged out. All review-derived figures now aggregate from the 0.10.0 persistent store (`ReviewStore::list_reviews`, same source as the History page), with the in-memory store kept only for `activeQueue` (live pending+running queue state — the correct source for that card) and as the fallback when persistence is disabled (`REVIEW_DISABLE_DB=1`), mirroring the `/reviews` handler's DB-first/in-memory split. A failed history read returns 500 like `/reviews`, never a page of fabricated zeros. (`src/server/api/dashboard.rs`)
- **Trend deltas are real and honest about missing data**: `reviewsTrend` (this week vs last week), `successTrend` (today vs yesterday), and `durationTrend` (this week's average duration vs last week's) are computed from the DB instead of the hardcoded `0.0` placeholders that rendered as fake "较上周/较昨日 ±0%" deltas. Every delta is `null` when its comparison window has no data and the KPI cards render "—" (`successTrend` is a percentage-point delta; the count/duration deltas are relative percent). `successRate` and `avgDurationMs` are likewise `null` when the current week has no data instead of pretending 100%/0ms. Windows are anchored in the server's local timezone (set `TZ` in containers): "this week" is the current ISO week (Monday 00:00 local), matching the card labels; the 24h trend keeps its existing rolling-hourly-bucket shape, now sourced from DB `created_at`. `recentReviews` now comes from the DB (latest 5, `created_at` DESC). Poll cost stays low for the 60s refresh: counts are SQL `COUNT(*)` totals, only averages/bucketing/recent materialize rows (capped at the newest 500 per window). (`src/server/api/dashboard.rs`, `frontend/src/types/dashboard.ts`, `frontend/src/components/Dashboard/KpiCard.vue`, `frontend/src/views/Dashboard.vue`, `docs/rest-api.md` §8)
- **System-health integration detection reads actual git platform configuration**: the GitLab/GitHub "integration" rows grepped LLM provider **names** for "gitlab"/"github", so a working integration showed "offline" unless an LLM happened to be named after it. Detection now consults the configured git platform instances (`git_platforms`, hot-mirrored in `AppState.git_platforms`) — GitLab reports `success` when any `type=gitlab` platform is configured, GitHub likewise for when that platform type lands. Follow-up fix on the same release: the env/CLI credential channel (`GITLAB_TOKEN` / `--gitlab-token`, `GITHUB_TOKEN` / `--github-token`) wires webhook/MR-fetch clients directly and never appears in `git_platforms`, so `AppState` now records startup flags for both and the dashboard health ORs them in — an env-configured integration on a real deployment no longer reads as 离线 while working. The always-`0` `latencyMs` field was dropped from every health row (a fabricated measurement; on-demand latency probing remains available via `POST /api/v1/llm/providers/{id}/test`). (`src/server/api/dashboard.rs`, `src/server/state.rs`, `src/cli/app.rs`, `frontend/src/views/Dashboard.vue`, `frontend/src/types/dashboard.ts`, `docs/rest-api.md` §8)

## [0.10.7] - 2026-09-11

### Changed
- **Instant-save everywhere in the WebUI — edit modes removed (RENG-41)**: the Configuration and Experts pages no longer use two-phase "enter edit mode → edit → save" flows; every change now persists itself.
  - **Configuration page**: the 编辑配置/保存/取消 buttons, edit-mode gating, unsaved-changes navigation guard, and beforeunload prompt are gone — all fields are always editable. A 500 ms debounced auto-save (`useConfigForm`) PUTs the full config (minus the LLM section, same payload contract as the old manual save) after any user-driven change; the initial fetch/population never triggers a save because the save fires only when the form differs from the last-persisted snapshot (dirty guard). A small header indicator shows 保存中… / 已自动保存 / 保存失败（下次修改时重试）; a failed save keeps the edits in the form (never silently dropped) and the next edit retries automatically. The excluded-pattern transient row commits on Enter/blur (auto-save picks it up) and Esc discards it without saving. Background auto-refresh (10 s) pulls external config changes but refuses to overwrite dirty or in-flight local edits.
  - **Experts page**: the page-level 编辑模式 toggle, its confirm notifications, the unsaved-changes router guard, and the per-card 编辑 dialog are removed. Card enable-switches apply immediately with optimistic UI + rollback on failure; weight slider drags debounce (500 ms) into a single `PUT` per drag, also with rollback. A failed update notifies and reverts the card.
  - **Generic auto-refresh composable**: `useAutoRefresh(fetchFn, intervalMs)` (extracted from the RENG-40 history-page polling) provides setInterval polling that pauses while the tab is hidden, never overlaps in-flight fetches, and refreshes immediately on return to visibility. It now backs the History page (`useReviews` refactored to delegate, same public API), Configuration (10 s), Experts (10 s, skipped mid-weight-save so polls can't snap a slider back), Queue Monitor (3 s, replacing its hand-rolled interval), and LLM Status (30 s, replacing its hand-rolled timer).
  - **i18n**: removed now-unused keys across all six locales (`common.saveChanges`, `config.editBtn`/`subtitle.edit`/`noChangesToSave`/`unsaved.*`/`saved`/`validation.title`/`validation.fixBeforeSave`/`validation.saveWithWarnings`, `experts.editMode*`/`doneEditing*`/`editTitle`/`edit.*`/`savedTitle`/`savedMessage`/`unsavedConfirm`/`editAria`/`cancelEditAria`/`saveChangesAria`); added `config.autoSave.{saving,saved,failed}` and `experts.updateFailed`. (`frontend/src/views/Configuration.vue`, `frontend/src/views/ExpertsManagement.vue`, `frontend/src/views/QueueMonitor.vue`, `frontend/src/views/LlmStatus.vue`, `frontend/src/views/ReviewHistory.vue`, `frontend/src/components/Config/GitPlatformsSection.vue`, `frontend/src/components/ExpertsManagement/ExpertCard.vue`, `frontend/src/composables/useConfigForm.ts`, `frontend/src/composables/useAutoRefresh.ts`, `frontend/src/composables/useReviews.ts`, `frontend/src/i18n/locales/*` ×6)

## [0.10.6] - 2026-09-10

### Added
- **Review history shows everyone involved in the MR/PR, not just the commit author (RENG-42)**: the history table's first identity column becomes `参与者` / Participants and renders a GitLab-style overlapping avatar stack — head-commit author, then the MR/PR opener, then every other participant including commenters and robot accounts — with a hover list naming each one and its role (`作者` / `开启人` / `参与者`, plus a marker for bots); more than four participants collapse into a `+K` badge while the hover list still lists **all** of them, and records predating this release keep the previous single-author cell unchanged. Participants are collected once per review, deduplicated by user id → username → name with the highest role winning (`author` > `creator` > `participant`), and snapshotted into `reviews.source_meta`. GitLab gains `fetch_participants` (`GET /projects/:id/merge_requests/:iid/participants`); the head-commit author's avatar is resolved by matching that commit author against the participant list (no extra endpoint); `NoteAuthor` now keeps `avatar_url` and a bot flag, and migration `0003_participant_meta.sql` adds `mr_discussions.author_id` / `author_avatar_url` / `author_bot` so commenters recovered from stored notes carry an identity. Bots are detected from GitLab's explicit `bot` flag or a `_bot` username suffix, and from GitHub's `user.type == "Bot"`. Every provider call is best-effort: a failed participants lookup logs a WARN and the review completes with whatever was known, never failing. Also fixes a long-standing mismatch where the history author column took its **name** from the head commit but its **avatar** from the MR opener (`src/server/gitlab/hooks.rs`), i.e. two different people on one row — the avatar is now only carried over when it belongs to the commit author, and the MR opener is represented as a participant with role `creator`. `GET /reviews` items and `GET /reviews/{id}` gain `participants: [{name, username, avatarUrl, role, bot}]` (`[]` for records saved before this release); the old `author{name, avatarUrl}` field is kept for compatibility. Known limits: GitHub participants have no `author` role (only opener + commenters/reviewers — its commit-author lookup was deliberately left out), participants recovered from stored notes fall back to the note handle for the display name, and legacy rows keep the single-author rendering. (`src/models/mod.rs`, `src/git_provider/gitlab/client.rs`, `src/git_provider/github/client.rs`, `src/server/gitlab/hooks.rs`, `src/server/github.rs`, `src/server/task_queue.rs`, `src/server/api/types.rs`, `src/server/api/review/task.rs`, `src/server/api/review/discussion.rs`, `src/store/{mod,rows,sqlx,traits}.rs`, `migrations/0003_participant_meta.sql`, `frontend/src/views/ReviewHistory.vue`, `frontend/src/views/Dashboard.vue`, `frontend/src/services/reviews.ts`, `frontend/src/types/history.ts`, `frontend/src/i18n/locales/*` ×6, `docs/rest-api.md`)

## [0.10.5] - 2026-09-10

### Changed
- **Review history auto-refresh and avatar alignment (RENG-40)**: the `评审历史` page now polls every 5s so new reviews appear without manual refresh, pauses when the tab is hidden, and refreshes immediately on focus — while preserving the current search/filter/pagination/scroll state. Background refreshes are silent (`fetchReviews(..., silent = true)`): they never flip the page-level `loading` flag that the view binds to a full-page skeleton (`v-if="loading"` swaps the whole `el-table` out), so an ordinary poll used to flash a skeleton and reset the table's scroll position every 5 seconds. Measured in headless Chrome with 300 ms of emulated latency: the pre-fix build remounted the skeleton twice per 11 s idle window and dropped a seeded `scrollTop` of 300 back to 0, while the fixed build did neither (scroll stayed at 300) — a review submitted while the page was open showed up on its own after 5.5 s in both builds. A failed background poll is now swallowed instead of writing `error` and nulling the list, so one transient network error no longer blanks the table the user is reading; interactive loads (first paint, filter/page change, manual refresh) keep the skeleton and the error banner unchanged. The author avatar container is hardened (`overflow:hidden`, `img display:block`, centered initials fallback) and failed avatar images fall back to initials, eliminating the "floating" misalignment between rows. (`frontend/src/composables/useReviews.ts`, `frontend/src/views/ReviewHistory.vue`)
...
### Fixed
- **Webhook-triggered reviews ignored the WebUI-configured LLM providers**: the shared webhook review path (`run_review_common`, used by both the GitLab and GitHub webhook handlers) resolved LLM providers from only the config file's `[[llm]]` section and the `LLM_CONFIG` env — never the server's hot-applied `state.llm_configs` (populated from the DB `llm_providers` table and updated live by the WebUI / `PUT /api/v1/config`). A provider configured in the WebUI therefore had no effect on webhook-dispatched reviews, which failed with `all experts failed: ... LLM config 'default' has no api_base set`. Both webhook handlers now snapshot `state.llm_configs` at dispatch time and pass it down; resolution precedence is config file `[[llm]]` > server (hot-applied) > `LLM_CONFIG` env, an empty server list counts as not provided (same semantics as the 0.9.1 API-path fix), and the CLI path (no server state) is unchanged. When all three sources are empty the review logs a clear ERROR pointing at the WebUI/provider configuration before the task fails. A new integration test drives the full webhook flow — mock GitLab instance + provider added via `POST /api/v1/llm/providers` with no `LLM_CONFIG` env and no config file — and asserts the review completes by calling the hot-applied provider. (`src/server/mod.rs`, `src/server/gitlab/handler.rs`, `src/server/gitlab/hooks.rs`, `src/server/github.rs`, `tests/server/llm_configs.rs`)
...
### Added
- **GitLab System Hook (admin-level) support**: webhooks from an admin-configured system hook are now handled — `X-Gitlab-Event: System Hook` payloads are routed by their event type (GitLab 19+ sends `event_type`, older versions/`event_name`; both supported, `event_name` preferred, unknown events ignored), and the instance-URL extraction chain falls back through `project.homepage` → `repository.homepage` → `object_attributes.url` because system-hook payloads carry no `project.web_url`. MR/Note handlers read the full MR URL from `object_attributes.url` (project-level webhooks keep the existing `project.web_url + iid` construction). One admin-level hook can now drive reviews for every project on an instance. Verified end-to-end against GitLab EE 19.2.4: signed system hook → verification → routing → full review → notes posted back to the MR. (`src/server/gitlab/handler.rs`, `src/server/gitlab/hooks.rs`)
- **Per-platform project allowlist** (`allowedProjects` in the Web UI Git 平台 form, `allowed_projects` in config): a `path_with_namespace` allowlist gates webhook-triggered reviews — events from projects not listed are answered with `200 {"status":"ignored","reason":"project not in allowlist"}` (hot-effective, no restart). Empty allowlist = every project allowed. REST `gitlab_mr` routing is unaffected (the allowlist gates inbound webhook dispatch only). (`src/models/git_platform.rs`, `src/server/api/config/put.rs`, `frontend/src/components/Config/GitPlatformsSection.vue`)
- **Encrypted secrets at rest**: `token` / `webhook_secret` / `webhook_signing_secret` (git platform entries and the legacy `[gitlab]` section) are encrypted in `ui-state.toml` as `enc:<base64(nonce‖ciphertext‖tag)>` (ChaCha20-Poly1305) under a per-config-dir key file `secrets.key` (32 random bytes, `0600`, auto-created on first save). Existing plaintext files load transparently and are encrypted on the next save. Losing `secrets.key` makes stored secrets unrecoverable — back it up with `ui-state.toml`; on loss, re-enter the secrets in the Web UI. (`src/config/secrets.rs`, `src/server/api/config/persist.rs`)
- **Per-platform internal URL** (`internalBaseUrl` in the Web UI, `internal_base_url` in config): the payload URL (external `base_url`) identifies the instance for webhook verification, while the optional internal URL is used for review-time API fetches — the container-reachable endpoint (e.g. `http://host.docker.internal:8929` locally, or the in-network `https://gitlab.islet.space` on a NAS) where the external port-mapped URL may be unreachable. Falls back to `base_url`, then to the payload URL. A matched platform rewrites the review URL onto this endpoint before any GitLab API call. (`src/models/git_platform.rs`, `src/server/gitlab/hooks.rs`, `frontend/src/components/Config/GitPlatformsSection.vue`)

### Changed
- **`ui-state.toml` is now the authoritative source for GitLab credentials**: env/CLI (`GITLAB_TOKEN`, `GITLAB_WEBHOOK_SECRET`, `GITLAB_WEBHOOK_SIGNING_SECRET`, and their `--gitlab-*` flags) are deprecated to fallback-only — they are used only when the persisted file carries no value for that field, and each use logs a deprecation warning. Webhook verification/review credentials are hot-effective via the Web UI Git 平台 entries, no restart and no env required. (`src/server/api/config/persist.rs`, `src/cli/app.rs`)
...
### Changed
- **「添加/编辑 Git 平台」dialog secret-field UX**: a secret that already has a stored value (returned by `GET /config` as the `***` mask) now shows a `••••••••••` placeholder in edit mode, so a filled field is visually distinguishable from an empty one; the two webhook help texts moved from static lines under the inputs into hover/focus tooltips behind a ⓘ icon next to the field label (focusable, keyboard-accessible); the card's static subtitle line ("管理用于接收 Webhook 并触发评审的 Git 平台实例…") is removed (its i18n key dropped from all 6 locales). (`frontend/src/components/Config/GitPlatformsSection.vue`, `frontend/src/i18n/locales/*` ×6)

### Fixed
- **「Git 平台」row「测试」button was unclickable in view mode**: the page's `<el-form :disabled="!isEditing">` cascaded `disabled` into every button without an explicit prop; the connectivity probe is read-only and now carries `:disabled="false"`, which short-circuits Element Plus' `useFormDisabled` (`props.disabled ?? … ?? form.disabled`) so testing a saved platform no longer requires entering edit mode. Edit/remove buttons stay gated behind edit mode as before. (`frontend/src/components/Config/GitPlatformsSection.vue`)
...
### Fixed
- **Docker entrypoint now version-aware syncs stale bind volumes (docker pull 后自动覆盖卷内旧二进制)**: the entrypoint previously only synced the image contents when the volume was empty, so after `docker pull` of a newer image the `./bin` / `./frontend-dist` bind volumes kept running the old binary (user NAS reproduction: image v0.9.14, volume v0.9.12 — Web UI showed the old version and upgrade reported *no asset*). The entrypoint now parses and compares the image binary version against the volume binary version on startup (semver, `--version`): empty volume → first-boot sync; image > volume → overwrite binary and cascade-sync frontend dist; volume >= image → keep the volume (never downgrades an in-container self-upgrade). Fail-safe: if either `--version` parse fails, the volume is left untouched, a WARN is printed and startup continues — the sync logic can never prevent the container from starting. Covered by a self-contained smoke test (`scripts/entrypoint-sync-smoke.sh`, 6 scenarios, no docker build required). (`entrypoint.sh`, `scripts/entrypoint-sync-smoke.sh`, `docker-compose.yml`, `deploy/gitlab-ee.md`)
- **Dispatcher wait() bounded with timeout (aborted review tasks become recoverable)**: `MrDispatcher::wait` previously blocked on the task-change channel with no deadline, so when a review task was aborted and never called `complete()`/`reset()`, the waiting worker stayed wedged and the MR could never be re-reviewed. `wait` now wraps the wait in `tokio::time::timeout` (default 15 minutes) and returns a `WaitOutcome`, so an aborted task's caller can reclaim the slot and `try_start` again. (`src/server/dispatcher.rs`)
- **Config sparse PUT no longer silently zeroes omitted fields (partial-update semantics)**: `PUT /config` previously replaced the whole stored config (`*ui = body`), so a request carrying only one section reset every omitted field to a serde default — silently zeroing `temperature` / `minScore` / `maxConcurrentReviews` / `enableMetrics` or dropping `llm.providers`. The endpoint now deep-merges the request JSON over the stored config key-by-key: fields present in the request overwrite, omitted fields keep their stored value, and a full-form PUT deep-merges to exactly the old behavior. Covered by a sparse-PUT regression test (only `rules.minScore` present). (`src/server/api/config.rs`)
- **GitHub note hook `/review` exact match (prefix lookalikes no longer trigger)**: a GitHub issue-comment starting with `/review` as a prefix — e.g. `/reviewer`, `/reviewxyz` — previously started a review. The matcher now requires the command at a path-segment boundary (`^/review(/|$)` semantics): `/review` and `/review/123` trigger, `/reviewer` / `/reviewxyz` / `/review @someone` do not; `/describe` gets the same rule. (`src/server/github.rs`)
- **GitLab hook exact match + apiToken masking (secret no longer echoed to the UI)**: GitLab comments get the same path-segment boundary fix as GitHub, and `GET /config` now returns the GitLab API token as the `***` mask instead of the live value — the frontend treats `***` as "keep unchanged" on save, and a token configured outside the UI (CLI `--gitlab-token` / env) is surfaced as the mask so an unrelated save can never silently wipe it. (`src/server/gitlab.rs`, `src/server/api/config.rs`, `frontend/src/views/Configuration.vue`, `frontend/src/i18n/locales/*.ts`)

### Added
- **Configuration page partial save (missing fields warn instead of blocking)**: saving the Configuration page with missing/empty fields previously failed the whole save; it now warns and persists whatever is filled in, matching the backend's sparse-PUT semantics. (`frontend/src/views/Configuration.vue`, `frontend/src/i18n/locales/*.ts`)
...
- **API token & bootstrap guide rewritten for deployers**: `docs/faq.md`'s auth section is now a structured guide — a concept table (API Token vs Bootstrap Key vs env token), three startup modes compared (direct env / bootstrap key / loopback exception), first-run setup and token rotation step-by-step, a three-step 401 troubleshooting, and a glossary — matching the v0.9.14 semantics (non-loopback bind requires a token or bootstrap key or startup is refused; the bootstrap key doubles as the rotation-rescue credential; env token always takes precedence). `docker-compose.yml` bootstrap comments and `deploy/gitlab-ee.md` were aligned to the same terminology. (`docs/faq.md`, `docker-compose.yml`, `deploy/gitlab-ee.md`)
- **v0.9.14 multi-platform performance data (unified baseline, dual-version comparison)**: `docs/features.md` now carries v0.9.14 benchmark numbers across four platforms on a unified baseline, including a v0.9.12 vs v0.9.14 comparison. (`docs/features.md`)
- **Windows install guide**: `docs/getting-started.md` gains a Windows section covering installation and usage of the Windows build. (`docs/getting-started.md`)
- **README refresh (repo URL casing, perf data, Docker upgrade, promo links)**: `README.md` / `README.zh-CN.md` refreshed — repo URL casing corrected, perf data updated, Docker in-container upgrade flow surfaced, promo-site links consolidated. (`README.md`, `README.zh-CN.md`)
- **Deploy guide privacy — real NAS path replaced with placeholder**: `deploy/gitlab-ee.md` no longer carries a real NAS mount path copied verbatim from a user report; it uses a placeholder so deployments don't leak host paths. (`deploy/gitlab-ee.md`)
- **README API-token naming aligned (drop stale "API Key" alias)**: `docs/README.md` drops the stale "API Key" alias in favor of the canonical "API Token" naming. (`docs/README.md`)
...
### Added
- **Frontend internationalization (i18n) with a 6-language switcher**: the web UI now ships a complete vue-i18n setup — `frontend/src/i18n/index.ts` creates the singleton i18n instance (Composition API mode) whose initial locale is resolved by `frontend/src/i18n/locale.ts` (`detectInitialLocale` auto-detects from `navigator.language` and falls back to English), and `frontend/src/composables/useLocale.ts` exposes the reactive `locale` plus a matching `elementLocale`, persisted to `localStorage` on switch. A new `LanguageSwitcher` component (`frontend/src/components/common/LanguageSwitcher.vue`, with `frontend/src/assets/language-switch.svg`) sits in the top-right of the layout and cycles the app language immediately without a reload. The locale bundle covers English plus five non-English locales — Simplified Chinese (`zh-CN`), Traditional Chinese (`zh-TW`), Japanese (`ja`), Korean (`ko`), and French (`fr`) — where `en.ts` defines the canonical keyset (382 keys) and the other five backfill the full set. Every hardcoded string in `App.vue`, all seven views (`Dashboard`, `Configuration`, `ExpertsManagement`, `LlmStatus`, `QueueMonitor`, `ReviewHistory`, `SystemLogs`) and the shared components (`KpiCard`, `StatusBadge`, `ExpertCard`, `ProviderCard`, `TaskCard`, `UpgradeDialog`) was migrated to `$t()` keys, including script-side strings, and Element Plus components are localized through `<el-config-provider :locale="elementLocale">` so built-in labels, date pickers and pagination follow the selected language. (`frontend/src/i18n/**`, `frontend/src/composables/useLocale.ts`, `frontend/src/components/common/LanguageSwitcher.vue`, `frontend/src/App.vue`, `frontend/src/views/*.vue`, `frontend/src/main.ts`, `frontend/package.json`)

## [0.9.7] - 2026-08-10

### Added
- **`reng serve` native TLS (HTTPS) support**: `serve` now terminates HTTPS directly with `--tls-cert <path> --tls-key <path> --tls-port <port>` (default TLS port 8443), so self-hosted deployments no longer need an external reverse proxy (Caddy/nginx). TLS is built on `axum-server` with `rustls`/`rcgen` and stays fully optional — when no cert/key is passed the server remains pure HTTP on 8080 (fail-soft), and the TLS and HTTP listeners can run side by side for a zero-downtime migration. A new integration test boots the TLS listener alongside HTTP and asserts `/health` is served over both. (`src/cli/mod.rs`, `src/server/mod.rs`, `tests/server.rs`, `Cargo.toml`)
- **Docker deployment defaults to HTTPS**: `docker-compose.yml` now maps `${REVIEW_TLS_PORT:-443}:8443` by default and mounts the certificate and private key read-only into the container at `/app/tls/cert.pem` and `/app/tls/key.pem` (host paths overridable via `TLS_CERT_PATH`/`TLS_KEY_PATH`, with the `./tls/` directory gitignored so keys cannot be committed). TLS is toggled by the `REVIEW_TLS_CERT`/`REVIEW_TLS_KEY` env pair: both non-empty turns TLS on, otherwise the container stays pure HTTP; `entrypoint.sh` appends the `--tls-cert/--tls-key/--tls-port` flags only when both are set, and the image now `EXPOSE 443 8080`. Plain-HTTP host access is off by default behind a commented-out port mapping guarded by `REVIEW_ENABLE_HTTP=true`, while container port 8080 stays reserved for the healthcheck. The commented Caddy reverse-proxy block was removed in favor of the built-in TLS. (`Dockerfile`, `docker-compose.yml`, `entrypoint.sh`, `.env.example`, `.gitignore`)
- **GitLab EE deployment guide documents HTTPS**: `deploy/gitlab-ee.md` gains an HTTPS section covering self-signed certificate generation, the `REVIEW_TLS_*`/`TLS_*_PATH` wiring, webhook HTTPS, and an updated Security Checklist. (`deploy/gitlab-ee.md`)
...
### Fixed
- **Large-PR chunked reviews silently under-covered most of the diff and returned a fake high score (systemic, user-reported)**: a big diff split into chunks triggered the large-PR path, but `max_chunks_per_expert` was enforced as a *file* cap instead of a *chunk* cap, and the content-pattern route was exclusive — once a file matched one expert's pattern it vanished from every other expert's view. In the reported case 29 files produced only 4 actually reviewed (86.2% un-reviewed) while the report still scored a fake 85/100. `route_chunks` now counts the quota in chunks and preserves chunk boundaries, the content-pattern route is additive (never exclusive), and a mandatory round-robin coverage pass plus a balance pass guarantee every file is reviewed by at least one expert. New `FileCoverage` accounting is threaded through consolidation, caps the score proportionally (`score × reviewed / total`) so under-coverage can never inflate the result, and the Lead Summary renders a coverage banner naming the files no expert covered. Measured regression: coverage 4 → 29 files, findings 20 → 55, score honestly corrected from a fake 85 to 63. (`src/diff/large_pr.rs`, `src/team/orchestrator.rs`, `src/team/lead_consolidator.rs`, `src/output/team_renderer.rs`)
- **`reng review --diff` discarded `--local-path`**: the diff branch dropped the flag in the `..` catch-all, so the Full File Contents injection read from the placeholder "local" instead of the real checkout. The real path (defaulting to the current directory) now flows through `run_local`, restoring full-file context for chunked large-PR reviews. (`src/cli/mod.rs`, `src/cli/handlers.rs`)
- **`reng ask --stdin` swapped its arguments**: `run_ask_stdin` passed the stdin diff as the question and the question string as the diff path, so every non-empty stdin + `--question` failed with `No such file or directory (os error 2)`. A shared `run_ask_with_diff` helper now serves both the `--diff` and `--stdin` paths, so the argument order can never diverge again. (`src/cli/handlers.rs`)
- **`reng repo-review` / `reng audit` on a missing path produced a fabricated report**: `scan_dir` failed open on a nonexistent root (empty entries → a fake 53/100 report for a path that does not exist). It now fails closed with `Repository path does not exist: <path>`. (`src/repo/mod.rs`)
- **Chunker integer overflow and double token counting**: the token accumulator used plain `+`, so a pathological giant diff could overflow the `usize` accumulator, and `adaptive_chunk` re-tokenized the whole chunk when appending a file (tokens counted twice). Both paths now use saturating arithmetic / single-pass token accounting. (`src/diff/chunker.rs`)
- **Browser SSE log/event streams returned 401**: `EventSource` cannot send an `Authorization` header, so `/logs` and `/events` always hit the auth wall for the UI. The two SSE paths now accept `?token=` in the query string through a strict allowlist (`/logs`, `/events`, both slash forms); every other endpoint — including the NDJSON `/logs/download` — deliberately keeps header-only auth, so the weaker query-token mechanism is never a global auth vector. (`src/server/auth.rs`)
- **`GET /config` leaked LLM API keys in plaintext**: a configured key was returned verbatim to the UI. `GET /config` now masks every configured key as `***`, and `PUT /config` treats an empty or masked key as "keep the stored value", so masking can never destroy state on save. (`src/server/api/config.rs`)
- **Provider "Test Connection" failed on an empty body with 400**: the backend `test_provider` handler extracted a required JSON body, so a bodyless POST was rejected with 400 before the test could run. The required extractor is removed and the frontend sends an explicit `{}` body. (`src/server/api/llm.rs`, `frontend/src/services/llm.ts`)
- **Deleting a provider then saving could not complete**: provider ids are index-derived, so deleting a lower-index provider shifts the list and the still-pending higher ids 404 (or worse, delete the wrong provider). The frontend now deletes in descending index order and, after the deletes, re-fetches the server list and zips the fresh ids onto the surviving providers, aborting with a clear error if the list changed mid-save. (`frontend/src/views/Configuration.vue`)
- **The Configuration page could not be saved**: required secret fields were validated as mandatory even though the backend keeps blank = unchanged, and an empty `requiredExperts` list made the form permanently invalid so Save was always disabled. Blank keys are now allowed ("leave empty to keep") and the required-experts list is backfilled from the currently enabled experts (falling back to the documented default trio), so the form starts valid. (`frontend/src/views/Configuration.vue`)
- **Queue max-concurrent stepper was dead**: Element Plus emits `change` only on Enter/blur; the +/- stepper buttons only emit `update:model-value`, so a `@change`-only binding never fired and the value looked unchanged. The stepper now commits on any model update through a short debounce, with Enter/blur committing immediately and cancelling the pending debounce. (`frontend/src/views/QueueMonitor.vue`)

### Added
- **Versioned test-case library under `tests/cases/`**: the two-phase test dispatch methodology (expectations written from code without running, then independently executed against the spec by a separate agent) now ships its case libraries in-repo instead of in the gitignored `reports/`: `cli-test-cases-v0.9.4.csv` (135 cases covering every subcommand's full parameter-combination matrix — happy paths, missing required args, invalid values, mutually exclusive combos, boundary values — with exit-code + key-string expectations derived from `src/cli/mod.rs` and `src/cli/handlers.rs`), `ui-test-cases-v0.9.4.csv` (136 per-page layout and interaction cases), and a `README.md` documenting the version mapping, judging rules (exact exit code + key substring), and the `reng upgrade` test seams. Case files are versioned with the code version so behavior changes diff cleanly across releases. (`tests/cases/`)
