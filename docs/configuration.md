...
**Communication latency** is a third number and, since RENG-78, the one the card shows first: the mean round trip of the probes themselves (`avgProbeLatencyMs`), recorded per probe in `llm_probe_samples` — one `GET {api_base}/models` each, with no model involved, so it measures the network alone. `reng serve` probes every enabled provider on its own every 30 minutes (one-shot CLI commands do not), which is what gives the average a history; the card labels it 平均通信延迟 / Avg comm. latency and falls back to the recorded call average while no probe average exists. Call latency is the only one of the three that contains model generation, which is why it is no longer the headline.
...
The **Test (测试)** button on a Git platform row probes that instance's connectivity (`GET {target}/api/v4/version`, 10s timeout). The probed target follows the same rule the review fetch itself uses (RENG-101): the entry's **internal URL when set** (the address the server actually reaches, e.g. the container-internal 443), falling back to **Base URL** — so the verdict reflects how the review will fetch, not just the address you browse to. The result toast names the probed address (`probedUrl`). Health recording still keys on the entry's external `base_url`, so the dashboard's 系统健康 keeps reporting this entry.

Correspondingly, a history row's MR link (「查看原始评论」) opens the **external (browseable) address** — what a webhook row always stored — while the review itself fetches via the internal one. Rows written before this fix stored the internal address; they are re-hosted onto the entry's external `base_url` for display only, never rewritten in storage.
...
**The aggregation toggle (RENG-95).** The page's「聚合报告 / Aggregated report」switch controls the report-level flag `report.aggregated` — a flag the WebUI could not control before (only the config file could set it, and it defaulted to `false`). The aggregator expert runs only when **both** conditions hold: this flag is on **and** the `aggregator` expert is enabled (`select_aggregator_expert`). The aggregator's card shows the state on the row itself — a warning tag "已启用但聚合未开启" when it is enabled but the flag is off, a success tag when both are on — so the "12 enabled, 11 participating" surprise cannot recur silently. The switch flips the running config immediately and persists to the configuration database (the same `ui` row `PUT /config` writes; a config-page save that never mentions aggregation does not reset it). Without a database the change is memory-only and the response says so (`persisted: false`).
