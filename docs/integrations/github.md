...
- It posts inline comments on files and lines, chosen by the **inline-note delivery policy** (below). The candidate set is the **consolidated** finding set — deduplicated across experts and filtered by the adjudication pass — and, when the reviewed diff is available, a comment is only posted when its line is part of that diff (with no diff in hand the gate fails open and the provider has the final word). Each comment opens with its `` `path:line` `` anchor. Unlike GitLab, GitHub's comment API addresses a line by one side (`line` + `side=RIGHT`), so the old-side number an anchor may carry is ignored here (RENG-99).
...
- **A failed comment is visible in the report, not only in the log** (RENG-99): when a round loses comments — refused by GitHub, or stopped after their retries got no answer at all — the board is updated with an `## Inline notes — N could not be published` section naming each failed `` `path:line` ``, with its HTTP status when there was one, and GitHub's verdict. The publish failure is recorded on the review result too, so `GET /api/v1/reviews/{id}` reports it.
