...
- It posts inline notes on files and lines, chosen by the **inline-note delivery policy** below. The candidate set is the **consolidated** finding set — deduplicated across experts and filtered by the adjudication pass — and a note is only posted when its line is part of the reviewed diff. Each note opens with its `` `path:line` `` anchor.
...
- **A refused note is visible in the report, not only in the log** (RENG-99). When a round loses notes, the board is updated with an `## Inline notes — N could not be published` section naming each refused `` `path:line` ``, its HTTP status and GitLab's verdict, and the publish failure is recorded on the review result so `GET /api/v1/reviews/{id}` reports it too. Before this, the only trace was one `WARN` per note and a silently thinner review.
...
| Anchor | the line must be in the reviewed diff, in a shape GitLab's position lookup matches | A note the provider would reject is never sent — including the *shape*: an unchanged line whose `old_line` we do not know would earn a `400 line_code can't be blank` (RENG-99). |
