...
- A submitted URL whose `host[:port]` matches an entry's **Base URL** — or its **Internal URL**, when configured — is re-hosted onto that entry's **Internal URL** (falling back to **Base URL**) before the review runs. Path and query string are preserved; the host is compared case-insensitively, an explicitly written default port (80/443) folds to "unwritten", any other port is compared strictly, and the path / trailing slash never take part in the match. When no entry matches strictly, a URL whose host differs only in the port still matches the **unique** entry carrying that host (RENG-90) — a history row can carry the `external_url`'s port (`https://host:8443`) while the entry is configured port-less (or vice versa) — and is re-hosted onto that entry's own configured base.
- The rewritten URL is what the review fetches and what the persisted `request.source.url` replays. The task's recorded MR URL (`gitlabMrUrl`, RENG-101) is the **external display address** — the same path re-hosted onto the entry's **Base URL** (what a browser can open, matching webhook rows) — so the history link works while the fetch stays on the internal address. The address you typed is not otherwise stored (it is logged once at submit time with the platform it matched).
...
- It posts inline notes on files and lines, chosen by the **inline-note delivery policy** below. The candidate set is the **consolidated** finding set — deduplicated across experts and filtered by the adjudication pass — and, when the reviewed diff is available, a note is only posted when its line is part of that diff (with no diff in hand the gate fails open and the provider has the final word). Each note opens with its `` `path:line` `` anchor.
- **The `position` is shaped the way GitLab matches it** (RENG-99). GitLab derives a diff note's `line_code` itself — by looking the position up in its own diff with strict equality on the pair `(old_line, new_line)`, the line's unchanged side being `nil` server-side — so the client never sends a `line_code` (it is not an accepted field). An **added** line is addressed by `new_line` alone; a line the diff **leaves unchanged** is addressed by `old_line` *and* `new_line` together, and the old-side number comes from the diff the review ran on. Sending `new_line` alone for an unchanged line is what produced the deployed `400 … Note {:line_code=>["can't be blank", "must be a valid line code"]}` — the missing pair, not a missing `line_code`.
- A single failing note no longer stops the batch: permanent rejections are logged and skipped, transient provider errors are retried up to three attempts, and the run logs a `posted / rolled up / policy-excluded / anchor-ineligible / skipped / failed` summary. **The HTTP status decides** whether a failure is transient (RENG-65): 408/429/5xx are retried, every other status is permanent; only a failure with no HTTP answer at all — a transport failure — falls back to the message. A `404` whose response body mentions `connection closed` is therefore *not* retried.
- **A failed note is visible in the report, not only in the log** (RENG-99). When a round loses notes — refused by GitLab, or stopped after their retries got no answer at all — the board is updated with an `## Inline notes — N could not be published` section naming each failed `` `path:line` ``, with its HTTP status when there was one, and GitLab's verdict. The publish failure is recorded on the review result too, so `GET /api/v1/reviews/{id}` reports it. Before this, the only trace was one `WARN` per note and a silently thinner review.
- **A rejected note logs its cause** (RENG-71): one `WARN` per failure, naming the finding's `file:line`, the position fields that were submitted (`new_path` / `new_line`, plus `old_line` when the anchor carries one), the HTTP status and GitLab's response body — truncated to 512 characters by the same helper the LLM error samples use — so a `400 position is invalid` is distinguishable from a `403`, a `404` or a transport failure without reproducing it.
...
| Anchor | the line must be in the reviewed diff, in a shape GitLab's position lookup matches | When the reviewed diff is available, a note the provider would reject is never sent — including the *shape*: an unchanged line whose `old_line` we do not know would earn a `400 line_code can't be blank` (RENG-99). With no diff in hand the gate **fails open** (the provider then has the final word), which is why a note lost that way is reported in the board rather than silently dropped. |
