...
- Plain binary / Homebrew install: `~/.config/review-engine/` (or `$REVIEW_ENGINE_CONFIG_DIR` / `--config-dir` if you override it; `serve --data-dir <path>` moves the whole directory — see [Data directory](configuration.md#data-directory-serve---data-dir)).
