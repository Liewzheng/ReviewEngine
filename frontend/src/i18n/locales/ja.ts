...
export default {
  ...
  dashboard: {
    ...
    health: {
      ...
      storage: 'ストレージ',
      database: 'データベース',
      storageWritable: '書き込みテスト成功',
      storageHint: 'データベースへの書き込みが失敗しています。コンテナ内で reng doctor --fix を実行すると修復できます。',
  ...
  config: {
    ...
    providers: {
      ...
      retry: '再試行',
      retryAttempts: '再試行回数',
      disableThinking: '思考を無効化',
      disableThinkingHint:
        '推論フェーズをスキップします。推論モデルはそうしないと出力トークンを使い切って空の回答を返します。',
    ...
    providerCards: {
      ...
      deletePrimaryRequiresSelection:
        '「{name}」はチェーンの先頭（最初の有効なカード）です。先にほかのプロバイダーのカードを先頭へドラッグしてから削除してください。',
    ...
    gitPlatforms: {
      ...
      testOkUrl: '接続成功 — {url} — GitLab {version}',
      testFailed: '接続失敗 — {error}',
      testFailedUrl: '接続失敗 — {url}: {error}',
  ...
  experts: {
    ...
    detail: {
      ...
      promptSaving: '保存中…',
      promptSaved: '保存しました',
      promptSaveFailed: 'プロンプトの保存に失敗しました。編集は元に戻されました。',
      promptSourceConfig: 'このプロンプトは設定ファイル / 組み込みの既定値に由来します。ここで保存するとこのエキスパートにオーバーライドが適用されます。',
      promptSourceOverride: 'このプロンプトは WebUI のオーバーライドで、設定ファイルより優先されます。空のプロンプトを保存すると既定値に戻ります。',
