...
export default {
  ...
  dashboard: {
    ...
    health: {
      ...
      storage: '스토리지',
      database: '데이터베이스',
      storageWritable: '쓰기 테스트 통과',
      storageHint: '데이터베이스 쓰기가 실패하고 있습니다. 컨테이너에서 reng doctor --fix 를 실행하면 복구할 수 있습니다.',
  ...
  config: {
    ...
    providers: {
      ...
      retry: '재시도',
      retryAttempts: '재시도 횟수',
      disableThinking: '사고 비활성화',
      disableThinkingHint:
        '추론 단계를 건너뜁니다. 추론 모델은 그렇지 않으면 출력 예산을 모두 추론에 사용해 빈 답변을 반환합니다.',
    ...
    providerCards: {
      ...
      disabled: '"{name}" 공급자가 비활성화되었습니다',
      reordered: '공급자 순서가 저장되었습니다',
      ...
      deletePrimaryRequiresSelection:
        '"{name}"은(는) 체인의 선두(첫 번째 활성 카드)입니다. 다른 공급자 카드를 맨 앞으로 드래그한 뒤 삭제하세요.',
    ...
    gitPlatforms: {
      ...
      testOkUrl: '연결 성공 — {url} — GitLab {version}',
      testFailed: '연결 실패 — {error}',
      testFailedUrl: '연결 실패 — {url}: {error}',
      addDialogTitle: 'Git 플랫폼 추가',
      editDialogTitle: 'Git 플랫폼 편집',
      ...
      addedTitle: '플랫폼이 추가됨',
      addedMessage: '변경 사항을 저장하여 새 플랫폼을 확정하세요.',
    },
    rules: {
      title: '리뷰 규칙',
      minScore: '최소 리뷰 점수',
      maxDuration: '최대 리뷰 시간（초）',
      maxDurationUnlimited: '0 = 무제한',
  ...
  experts: {
    ...
    detail: {
      ...
      promptSaving: '저장 중…',
      promptSaved: '저장됨',
      promptSaveFailed: '프롬프트를 저장하지 못했습니다. 편집 내용이 되돌려졌습니다.',
      promptSourceConfig: '이 프롬프트는 설정 파일 / 기본 제공값에서 가져온 것입니다. 여기서 저장하면 이 전문가에 대한 오버라이드가 적용됩니다.',
      promptSourceOverride: '이 프롬프트는 WebUI 오버라이드로 설정 파일보다 우선합니다. 빈 프롬프트를 저장하면 기본값으로 복원됩니다.',
