// Simplified Chinese — full translation mirroring en.ts.
// Structure is kept strictly parallel to en.ts (same keys, same nesting).
// Named placeholders {n} {count} {title} ... are preserved verbatim.
export default {
  app: {
    name: 'Review Engine',
  },
  nav: {
    dashboard: '仪表盘',
    history: '历史记录',
    config: '配置',
    queue: '队列',
    llm: 'LLM',
    logs: '日志',
    experts: '专家',
  },
  header: {
    apiToken: 'API 令牌',
    updateAvailable: '发现新版本',
    updateAvailableTitle: '有新版本可用',
    upgrade: '升级',
    healthy: '运行正常',
    language: '语言',
  },
  common: {
    ok: '确定',
    cancel: '取消',
    close: '关闭',
    save: '保存',
    lastUpdated: '更新于 {time}',
    updateFailed: '更新失败',
    updateFailedAt: '更新失败 · {time}',
    reset: '重置',
    retry: '重试',
    remove: '移除',
    edit: '编辑',
    copy: '复制',
    logs: '日志',
    paused: '已暂停',
    optional: '可选',
    unknown: '未知',
    disabled: '已禁用',
    enabled: '已启用',
    keep: '保留',
    error: '错误',
    success: '成功',
    copied: '已复制',
    copyFailed: '复制失败',
    copyFailedMessage: '无法复制到剪贴板。',
    unavailable: '不可用',
    commandCopied: '命令已复制到剪贴板。',
    testConnection: '测试连接',
    lastTest: '上次测试：{date}',
    dismissTestResult: '关闭测试结果',
    status: {
      operational: '运行正常',
      degraded: '性能下降',
      error: '错误',
      offline: '离线',
      unknown: '未知',
      inProgress: '进行中',
      running: '运行中',
      queued: '排队中',
      completed: '已完成',
      failed: '失败',
      cancelled: '已取消',
      success: '成功',
      warning: '警告',
      skipped: '已跳过',
      info: '信息',
    },
  },
  token: {
    title: '需要 API 令牌',
    unlockHint: '此实例需要 API 令牌。请输入您配置的令牌以继续。',
    placeholder: '粘贴您的 API 令牌',
    tokenRequiredError: '请输入 API 令牌。',
    invalidToken: '服务器拒绝了该令牌。请检查后重试。',
    rotateTitle: '轮换 API 令牌',
    rotatePlaceholder: '粘贴新令牌（留空则保持当前令牌不变）',
    rotateHint: '新令牌将替换当前令牌，并立即生效。',
    rotateSave: '轮换令牌',
    rotateSuccess: 'API 令牌已更新。',
    rotateFailed: '更新 API 令牌失败。',
    rotateBootstrapKeyLabel: 'Bootstrap Key（可选）',
    rotateBootstrapKeyPlaceholder: '一次性引导密钥',
    rotateBootstrapKeyHint:
      '当前令牌无效时的自救轮换凭证——通过 REVIEW_BOOTSTRAP_KEY 或 --bootstrap-key 设置。',
    rotateInvalidHint:
      '当前令牌已被服务器拒绝。请输入上方的 Bootstrap Key 进行轮换，或修改 .env 中的 REVIEW_API_TOKEN 后重建。',
  },
  bootstrap: {
    title: '欢迎使用 Review Engine',
    intro:
      '此实例尚未配置 API 令牌。请创建一个以保护 API——令牌会保存到服务器并立即生效。',
    tokenLabel: 'API 令牌',
    tokenPlaceholder: '创建新的 API 令牌',
    tokenRequiredError: '请输入 API 令牌。',
    keyLabel: '引导密钥',
    keyPlaceholder: '一次性引导密钥',
    keyHint:
      '通过 REVIEW_BOOTSTRAP_KEY 或 --bootstrap-key 设置，并会打印在启动日志中。',
    keyRequiredError: '请输入引导密钥。',
    keyInvalidError: '引导密钥无效或缺失。请查看启动日志。',
    save: '设置令牌',
    saving: '正在设置…',
    saveError: '无法保存令牌，请重试。',
  },
  dashboard: {
    title: '仪表盘',
    subtitle: '系统概览与近期活动',
    kpis: {
      reviewsThisWeek: '本周评审',
      activeQueue: '活跃队列',
      successRate: '成功率',
      avgDuration: '平均耗时',
      vsLastWeek: '较上周',
      vsYesterday: '较昨日',
    },
    trend: {
      title: '活动趋势',
      total: '总计：{count} 次评审',
      empty: '过去 24 小时无活动',
      view24h: '24H',
      viewDaily: '按天',
      tooltipReviews: '{count} 次评审',
    },
    health: {
      title: '系统健康',
      integrations: '集成状态',
      notProbed: '未探测',
      notConfigured: '未配置',
      llmProviders: 'LLM 提供商',
      storage: '存储',
      database: '数据库',
      storageWritable: '写入测试通过',
      storageHint: '数据库写入失败，可在容器内执行 reng doctor --fix 自动修复。',
      allOperational: '所有系统运行正常',
      someDegraded: '部分系统性能下降',
      errorsDetected: '检测到系统错误',
    },
    recent: {
      title: '近期评审',
      viewAll: '查看全部',
      empty: '暂无近期评审',
    },
    time: {
      justNow: '刚刚',
      minAgo: '{n} 分钟前',
      hoursAgo: '{n} 小时前',
      daysAgo: '{n} 天前',
    },
  },
  history: {
    title: '评审历史',
    export: '导出',
    exportAria: '导出评审',
    exportEmptyTooltip: '当前筛选结果为空，没有可导出的评审记录',
    exportSuccess: '已导出 {count} 条评审记录',
    exportFailed: '导出失败',
    exportTruncated: '已导出前 {count} 条评审记录（共 {total} 条匹配，已达导出上限）',
    searchPlaceholder: '搜索 MR 标题、作者、分支…',
    empty: '未找到评审',
    untitledReview: '未命名评审',
    filters: {
      allProjects: '所有项目',
      allStatuses: '所有状态',
      allRepositories: '所有仓库',
      resetAria: '重置筛选',
    },
    date: {
      rangeSeparator: '至',
      start: '开始',
      end: '结束',
    },
    columns: {
      mrTitle: 'MR 标题',
      project: '项目',
      // RENG-45：历史表首个身份列改为「参与者」；authorName 为单作者列（仪表盘）。
      author: '参与者',
      authorName: '作者',
      status: '状态',
      score: '得分',
      duration: '耗时',
      created: '创建时间',
      time: '时间',
      actions: '操作',
    },
    llm: {
      unknown: '未知',
      expertTooltip: '实际生成该报告的 LLM（provider/model）',
    },
    participants: {
      roleAuthor: '作者',
      roleCreator: '开启人',
      roleParticipant: '参与者',
      bot: '机器人',
      more: '+{n}',
    },
    actions: {
      rerun: '重新评审',
      more: '更多操作',
      viewComment: '查看原始评论',
      copyId: '复制评审 ID',
    },
    rerun: {
      title: '重新评审',
      confirmBtn: '重新评审',
      confirmNoContext:
        '重新评审该任务？将使用相同参数创建新任务。',
      confirmWithMr:
        '重新评审“{title}”？这将向 MR 发布一条新评论。',
      requeuedTitle: '评审已重新排队',
      requeuedMessage: '已为 {title} 排队新的评审。',
    },
    llmNotConfiguredTitle: '未配置 LLM',
    llmNotConfiguredBody: '审核需要可用的 LLM 配置。是否前往配置页面设置？',
    goToConfig: '前往配置',
    copy: {
      idCopied: '评审 ID {id} 已复制到剪贴板。',
    },
    comment: {
      unavailable: '原始评论 URL 不可用。',
    },
    pagination: {
      empty: '显示 0 条评审',
      range: '显示第 {start} 到 {end} 条，共 {total} 条评审',
      perPage: '每页 {n} 条',
    },
    drawer: {
      reviewDetails: '评审详情',
      author: '作者',
      project: '项目',
      branch: '分支',
      created: '创建时间',
      duration: '耗时',
      commit: '提交',
      expertResults: '专家结果',
      tabs: {
        summary: '摘要',
        fullComment: '完整评论',
        apiResponse: 'API 响应',
      },
      generatedByPrefix: '由 {count} 位专家生成评审，总体状态',
      viewOnGitlab: '在 GitLab 中查看',
      score: '总体得分',
      noComment: '该审核未生成评论',
      rawResponse: '原始响应',
      unverified: '未验证',
      unverifiedTooltip: '所有专家均未发现问题或覆盖率不足——该得分不是经过验证的质量信号。',
    },
    riskLevel: {
      healthy: '健康',
      low: '低',
      lowMedium: '中低',
      medium: '中',
      high: '高',
      critical: '严重',
    },
    time: {
      justNow: '刚刚',
      minutesAgo: '{n} 分钟前',
      hoursAgo: '{n} 小时前',
      daysAgo: '{n} 天前',
    },
  },
  queue: {
    title: '队列监控',
    subtitle: '实时评审任务队列',
    resumeQueue: '恢复队列',
    pauseQueue: '暂停队列',
    maxConcurrent: '最大并发',
    maxConcurrentTitle: '设置最大并发任务数',
    cancelAllFailed: '取消所有失败任务',
    resumed: '队列已恢复',
    paused: '队列已暂停',
    maxConcurrentSet: '最大并发数已设为 {n}',
    noFailedToCancel: '没有可取消的失败任务',
    confirmTitle: '确认',
    cancelAllConfirm: '取消所有 {count} 个失败任务？',
    cancelAllBtn: '全部取消',
    cancelAllResult: '已取消 {succeeded} 个任务，{failed} 个失败',
    cancelConfirm:
      '取消对“{title}”的评审？此操作无法撤销。',
    cancelTitle: '确认取消',
    cancelReviewBtn: '取消评审',
    taskCancelled: '任务已取消',
    taskQueuedForRetry: '任务已排队等待重试',
    viewLogs: '查看任务 {id} 的日志',
    stats: {
      active: '活跃任务',
      queued: '排队任务',
      failed: '失败任务',
      depth: '队列深度',
    },
    sections: {
      active: '活跃任务',
      queued: '排队任务',
      failed: '失败任务',
      cancelled: '已取消任务',
      completed: '最近完成任务',
    },
    empty: '队列中没有任务',
    emptyHint: '队列当前为空。新任务将显示在这里。',
    task: {
      expert: '专家：{name}',
    },
    errors: {
      toggle: '切换队列失败',
      maxConcurrent: '更新最大并发数失败',
      cancelAllFailed: '取消所有失败任务失败',
      cancel: '取消任务失败',
      retry: '重试任务失败',
    },
  },
  config: {
    title: '配置',
    subtitle: '管理 Review-Engine 设置 — 更改将自动保存',
    loadFailed: '加载配置失败',
    autoSave: {
      saving: '保存中…',
      saved: '更改已自动保存',
      failed: '保存失败 — 已保留更改，将在下次修改时重试',
    },
    saveFailed: '保存配置失败',
    notSet: '（未设置）',
    llmNotConfiguredBanner:
      '尚未配置可用的 LLM — 审核需要 LLM 才能运行。请在下方「LLM 设置」填写 API 基础 URL 与密钥，或在「其他 LLM 提供商」中添加。',
    validation: {
      invalidUrl: '请输入有效的 URL',
      modelRequired: '必须填写默认模型',
      expertRequired: '至少需要一个专家',
    },
    llm: {
      title: 'LLM 设置',
      apiBaseUrl: 'API 基础 URL',
      apiBasePlaceholder: 'https://api.openai.com/v1（留空则保留原值）',
      apiKey: 'API 密钥',
      apiKeyPlaceholder: 'sk-...（留空则保留原值）',
      defaultModel: '默认模型',
      selectModelPlaceholder: '选择模型',
      maxTokens: '最大输出令牌数',
      temperature: '温度',
      timeout: '超时（秒）',
      retryAttempts: '重试次数',
      connected: '已连接 — {n}ms',
      testFailed: '失败 — {error}',
    },
    providers: {
      title: '其他 LLM 提供商',
      addBtn: '添加提供商',
      empty: '未配置其他提供商',
      collapse: '收起',
      notSaved: '尚未保存',
      providerType: '提供商类型',
      defaultModel: '默认模型',
      apiBaseUrl: 'API 基础 URL',
      apiKey: 'API 密钥',
      maxTokens: '最大令牌数',
      temperature: '温度',
      timeoutShort: '超时（秒）',
      retry: '重试',
      retryAttempts: '重试次数',
      disableThinking: '禁用思考',
      disableThinkingHint:
        '跳过推理阶段。推理模型否则会把全部输出预算耗在思考上，返回空答案。',
      modelPlaceholder: 'gpt-4o、claude-3、…',
      apiBasePlaceholder: 'https://api.openai.com/v1',
      apiKeyPlaceholder: 'sk-...',
      addDialogTitle: '添加 LLM 提供商',
      addedTitle: '提供商已添加',
      addedMessage: '保存更改以持久化新提供商。',
      saved: '提供商已成功保存',
      saveFailed: '保存提供商失败',
      saveOnlyConfirm:
        '主配置存在校验错误。仅保存提供商更改？',
      saveOnlyBtn: '仅保存提供商',
      removeTitle: '移除提供商',
      removeConfirm: '移除“{name}”提供商？',
      customProvider: '自定义',
      catalogUnavailable: '提供商目录不可用——已改用内置预设列表。',
      modelsCount: '{count} 个模型',
      viewDocs: '文档',
      modelsFallback: '模型列表不可用——请手动输入模型 ID。',
    },
    providerCards: {
      add: '添加供应商',
      empty: '尚未配置任何 LLM 供应商',
      primaryBadge: '主',
      chainPosition: '链序 #{n}',
      duplicate: '复制卡片',
      duplicateUnchanged: '未做任何修改——已存在一张配置完全相同的卡片，仍要再创建一个副本吗？',
      enable: '启用',
      disable: '停用',
      enabled: '已启用“{name}”',
      disabled: '已停用“{name}”',
      reordered: '已保存供应商顺序',
      setPrimary: '设为主提供商',
      primarySet: '已将“{name}”设为主提供商',
      addTitle: '添加供应商',
      editTitle: '编辑供应商',
      keepKeyPlaceholder: '留空则保留已保存的密钥',
      advanced: '高级设置',
      deleteTitle: '删除供应商',
      deleteConfirm: '删除供应商“{name}”？此操作将立即保存。',
      deletePrimaryRequiresSelection:
        '“{name}”是链首（第一张启用的卡片），无法直接删除——请先把其他供应商的卡片拖到最前，再删除它。',
      deleted: '已删除供应商“{name}”',
      saved: '供应商已保存',
      saveFailed: '保存供应商失败',
      duplicateProvider: '该供应商已配置',
      providerRequired: '请选择供应商类型',
      apiKeyRequired: '请填写 API 密钥',
      testUnavailable: '该供应商未在服务端激活',
    },
    gitPlatforms: {
      title: 'Git 平台',
      addBtn: '添加平台',
      empty: '尚未配置 Git 平台',
      name: '名称',
      namePlaceholder: '例如 gitlab-main',
      type: '类型',
      baseUrl: 'Base URL',
      baseUrlPlaceholder: 'https://gitlab.example.com',
      internalBaseUrl: '内部地址',
      internalBaseUrlPlaceholder: '容器内实际可达地址（可选）',
      internalBaseUrlHelp:
        'external（Base URL）是 GitLab payload 中出现的地址，用于 webhook 匹配；internal 是 reng 实际可访问的地址（如容器内网），用于拉取评审数据。留空则使用 Base URL。',
      token: '访问令牌',
      tokenPlaceholder: 'glpat-...',
      keepTokenPlaceholder: '留空则保留已保存的 Token',
      webhookSecret: 'Secret token',
      webhookSigningSecret: 'Signing token',
      keepSecretPlaceholder: '留空则保留已保存的密钥',
      webhookSecretHelp:
        '对应 GitLab webhook 表单的 Secret token 字段，以明文放在 X-Gitlab-Token 头中发送，安全性弱于 Signing token，作为可选兜底。两者可只配其一，至少配一个。',
      webhookSigningSecretHelp:
        'GitLab 19.0+ 的 Signing token（whsec_ 开头，GitLab 生成时仅显示一次）。GitLab 用它对请求体计算 HMAC-SHA256 并通过 webhook-signature 头发送，推荐配置。',
      allowedProjects: '允许的项目',
      allowedProjectsPlaceholder: '每行一个项目路径（如 group/project），留空 = 全部',
      allowedProjectsHelp:
        '系统级 hook 会覆盖所有项目，用此白名单限定参与评审的项目。留空 = 全部项目。',
      test: '测试',
      testOk: '连接成功 — GitLab {version}',
      testOkUrl: '连接成功 — {url} — GitLab {version}',
      testFailed: '连接失败 — {error}',
      testFailedUrl: '连接失败 — {url}: {error}',
      addDialogTitle: '添加 Git 平台',
      editDialogTitle: '编辑 Git 平台',
      nameRequired: '请输入名称',
      nameDuplicate: '已存在同名平台',
      baseUrlRequired: '请输入 Base URL',
      clearSecret: '清除',
      removeTitle: '移除平台',
      removeConfirm: '移除 Git 平台“{name}”？',
      addedTitle: '平台已添加',
      addedMessage: '保存更改以持久化新平台。',
    },
    rules: {
      title: '评审规则',
      minScore: '最低评审分数',
      maxDuration: '最大评审时长（秒）',
      maxDurationUnlimited: '0 = 不限制',
      blockOnCritical: '严重问题时阻止 MR',
      autoCommentOnPass: '通过时自动评论',
      commentTemplate: '评论模板',
      commentTemplatePlaceholder:
        "代码评审已完成。总体得分：{'{'}{score}{'}'}/100。{'{'}{summary}{'}'}",
      excludedPatterns: '排除的文件模式',
      addPattern: '添加模式',
      requiredExperts: '必需专家',
    },
    advanced: {
      title: '高级选项',
      hide: '隐藏高级',
      show: '显示高级',
      logLevel: '日志级别',
      logRetention: '日志保留（天）',
      sseHeartbeat: 'SSE 心跳间隔（秒）',
      maxConcurrent: '最大并发评审数',
      requestTimeout: '请求超时（秒）',
      enableMetrics: '启用指标',
      debugMode: '调试模式',
      storageBackend: '存储后端',
      storageBackendKind: {
        postgresql: 'PostgreSQL',
        sqlite: 'SQLite',
        disabled: '已禁用',
      },
    },
  },
  llm: {
    title: 'LLM 状态',
    subtitle: '提供商健康与性能',
    status: {
      healthy: '正常',
      degraded: '性能下降',
      error: '错误',
      offline: '离线',
      disabled: '已停用',
    },
    card: {
      disabled: '已停用',
      probeFailed: '探测失败',
      testOk: '测试成功',
      testFailed: '测试失败',
    },
    stats: {
      providers: '提供商',
      avgLatency: '平均延迟',
      avgLatencyWindow: '平均延迟（过去 {days} 天）',
      avgTtfb: '平均通信延迟',
      avgTtfbWindow: '平均通信延迟（过去 {days} 天）',
      avgCommLatency: '平均通信延迟',
      avgCommLatencyWindow: '平均通信延迟（过去 {days} 天）',
      totalRequests: '使用总数',
      totalRequestsWindow: '已记录使用（过去 {days} 天）',
    },
    metrics: {
      avgLatency: '平均延迟',
      avgTtfb: '平均通信延迟',
      avgCommLatency: '平均通信延迟',
      requests: '使用次数',
      successRate: '成功率',
    },
    latencySamples: '{n} 次调用 · {failed} 次失败',
    probeLatency: '探测 {n} ms',
    sparkline: {
      caption: '调用延迟 · 过去 {days} 天',
      label: '过去 {days} 天已记录的调用延迟',
    },
    usageShare: '占比 {percent}%（过去 {days} 天）',
    windowLabel: '过去 {days} 天',
    lastUsed: '最近使用：{date}',
    lastChecked: '上次检查：{date}',
    recentUsage: {
      title: '最近使用',
      chainHint: '链首：{name}',
      empty: '暂无已完成的评审',
      primaryUnused: '未使用首选',
    },
  },
  logs: {
    title: '系统日志',
    subtitle: '实时日志流',
    resume: '恢复',
    pause: '暂停',
    download: '下载',
    clear: '清空',
    levelsLabel: '级别：',
    filterPlaceholder: '筛选日志…',
    autoScroll: '自动滚动',
    formatLabel: '格式：',
    format: {
      relative: '相对',
      absolute: '绝对',
      iso: 'ISO',
    },
    showingCount: '显示 {filtered} / {total} 条日志',
    emptyCleared: '日志已清空。新条目将显示在这里。',
    emptyWaiting: '正在等待日志…',
    emptyFiltered: '没有符合当前筛选条件的日志',
    newLogs: '{count} 条新日志',
    clearConfirm:
      '清除可见日志？此操作仅影响显示，不影响已存储的日志。',
    clearTitle: '清除日志',
    downloadWarning:
      '下载的日志文件可能包含敏感信息，如 API 密钥或令牌。请小心处理并安全存储。',
    sensitiveTitle: '敏感内容警告',
    downloadStartedTitle: '下载已开始',
    downloadStartedMessage: '您的日志文件正在下载。',
    streamErrorTitle: '日志流错误',
    reconnecting: '连接中断，正在重连…',
    time: {
      secondsAgo: '{n} 秒前',
      minutesAgo: '{n} 分钟前',
      hoursAgo: '{n} 小时前',
    },
  },
  experts: {
    title: '专家管理',
    subtitle: '配置 LLM 评审专家',
    comingSoon: '即将推出',
    addExpert: '添加专家',
    addExpertComingSoonAria: '添加专家（即将推出）',
    stats: {
      active: '活跃专家',
      avgWeight: '平均权重',
      total: '专家总数',
    },
    searchPlaceholder: '搜索专家…',
    categoryPlaceholder: '类别',
    categories: {
      all: '全部分类',
    },
    empty: '未找到专家',
    emptyFiltered: '没有符合筛选条件的专家',
    clearFilters: '清除筛选',
    detailsTitle: '专家详情',
    detailsAria: '专家详情对话框',
    viewDetailsAria: '查看 {name} 的详情',
    card: {
      weight: '权重',
      viewDetails: '查看详情',
    },
    detail: {
      enabled: '已启用：',
      weight: '权重：',
      toggleAria: '切换 {name}',
      description: '描述',
      prompt: '提示词',
      promptSave: '保存',
      promptSaving: '保存中…',
      promptSaved: '已保存',
      promptSaveFailed: '提示词保存失败，编辑已还原。',
      promptSourceConfig: '该提示词来自配置文件 / 内置默认值。在此保存将为该专家应用覆盖。',
      promptSourceOverride: '该提示词是 WebUI 覆盖，优先于配置文件。保存空提示词将恢复默认值。',
      lastReviews: '最近 5 次评审',
      score: '分数',
      date: '日期',
    },
    toggle: {
      enabledTitle: '专家已启用',
      disabledTitle: '专家已禁用',
      enabledMessage: '{name} 已启用',
      disabledMessage: '{name} 已禁用',
    },
    updateFailed: '更新 {name} 失败，已还原更改',
    memoryOnlyTitle: '仅在内存中生效',
    memoryOnlyMessage:
      '{name} 已更新，但该服务未连接配置数据库，服务重启后该修改将丢失。请以数据库模式启动服务（指定数据目录，且不设置 REVIEW_DISABLE_DB=1），专家配置才能持久化。',
    aggregation: {
      label: '聚合报告',
      hint: '将所有专家的评审结果合并为一份聚合报告。仅当聚合器专家也已启用时生效。',
      toggleAria: '切换聚合报告',
      enabledTitle: '已开启聚合报告',
      enabledMessage: '下次评审将运行聚合器。',
      disabledTitle: '已关闭聚合报告',
      disabledMessage: '从下次评审起聚合器将不再运行。',
      updateFailed: '更新聚合报告设置失败，已还原。',
      memoryOnlyTitle: '仅在内存中生效',
      memoryOnlyMessage:
        '聚合报告设置已应用，但该服务未连接配置数据库，重启后该设置将丢失。请以数据库模式启动服务（指定数据目录，且不设置 REVIEW_DISABLE_DB=1）以持久化。',
      rowEnabledButOff: '已启用但聚合未开启',
      rowOn: '聚合报告已开启',
    },
  },
  upgrade: {
    title: '升级 Review Engine',
    checkFailed: '无法检查更新',
    releaseNotes: '发布说明',
    newerPrefix: '发现新版本',
    newerSuffix: '（当前版本：v{current}）',
    detectedPrefix: '检测到安装来源：',
    runInstruction: '要更新，请运行：',
    upgradeNow: '立即升级',
    binaryHint:
      '服务器二进制文件将被替换；需要重启才能生效。',
    noAsset:
      '此平台没有可用的发布包，无法自动升级。',
    dockerHint: '更新将在运行的容器内应用，完成后容器会自动重启。',
    doneFallback: '升级完成；需要重启才能生效。',
    dockerDone: '升级完成，容器即将自动重启。',
    failedFallback: '升级失败。',
    checkingLatest: '正在检查最新版本',
    inProgressError: '升级任务已在进行中，请稍后再试',
    notSupportedError: '此安装方式不支持自动升级。',
    restartHint: '容器正在重启以应用更新，页面将自动刷新。',
    downloadSpeed: '下载速度',
    downloadTotal: '总大小',
    downloadEta: '预计剩余',
    etaSeconds: '{s} 秒',
    etaMinutesSeconds: '{m} 分钟 {s} 秒',
    source: {
      binary: '预编译二进制',
      brew: 'Homebrew',
      docker: 'Docker 容器',
      cargo: 'cargo install',
      unknown: '未知安装方式',
    },
    step: {
      checking: '检查中',
      downloading: '下载中',
      verifying: '校验中',
      installing: '安装中',
      restarting: '重启中',
    },
  },
  errors: {
    unknown: '未知错误',
    downloadFailed: '下载失败',
    failedToLoadLogHistory: '加载日志历史失败',
    sseConnectionError: 'SSE 连接错误',
    failedToFetchModels: '获取模型列表失败',
    reviewNotFound: '未找到该评审，它可能已被删除。',
    reviewInProgress:
      '该评审仍在进行中，或原始请求已不可用，因此当前无法重新运行。',
    reviewParamsUnavailable: '存储的请求参数无法重放，因此无法重新运行该评审。',
  },
}
