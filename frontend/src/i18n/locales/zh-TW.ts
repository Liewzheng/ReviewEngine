...
export default {
  ...
  dashboard: {
    ...
    health: {
      ...
      storage: '儲存',
      database: '資料庫',
      storageWritable: '寫入測試通過',
      storageHint: '資料庫寫入失敗，可在容器內執行 reng doctor --fix 自動修復。',
  ...
  config: {
    ...
    providers: {
      ...
      disableThinkingHint:
        '略過推理階段。推理模型否則會把全部輸出預算耗在思考上，回傳空答案。',
    ...
    providerCards: {
      ...
      deletePrimaryRequiresSelection:
        '「{name}」是鏈首（第一張啟用的卡片），無法直接刪除——請先將其他供應商的卡片拖到最前，再刪除它。',
      deleted: '已刪除供應商「{name}」',
      saved: '供應商已儲存',
      saveFailed: '儲存供應商失敗',
    ...
    gitPlatforms: {
      ...
      testOkUrl: '連線成功 — {url} — GitLab {version}',
      testFailed: '連線失敗 — {error}',
      testFailedUrl: '連線失敗 — {url}: {error}',
  ...
  llm: {
    ...
    stats: {
      ...
      avgTtfb: '平均通訊延遲',
      avgTtfbWindow: '平均通訊延遲（過去 {days} 天）',
      avgCommLatency: '平均通訊延遲',
      avgCommLatencyWindow: '平均通訊延遲（過去 {days} 天）',
      totalRequests: '使用總數',
  ...
  experts: {
    ...
    detail: {
      ...
      promptSaving: '儲存中…',
      promptSaved: '已儲存',
      promptSaveFailed: '提示詞儲存失敗，編輯已還原。',
      promptSourceConfig: '此提示詞來自設定檔 / 內建預設值。在此儲存將為該專家套用覆寫。',
      promptSourceOverride: '此提示詞是 WebUI 覆寫，優先於設定檔。儲存空提示詞將還原預設值。',
