...
/// Run experts with the same API as the old `orchestrator::run_experts`.
///
/// Thin wrapper around [`run_experts_inner`] that discards metrics for
/// callers that only need the report list.
pub async fn run_experts(
    ...
    let (reports, _, _, _, global_context) =
        run_experts_inner(experts, mr_info, diff_raw, llm_configs, config, None, "").await?;
    Ok((reports, global_context))
}

/// Run the aggregator expert to consolidate multiple expert reports.
